...
{
  /// <summary>测试表CombGuid</summary>
  [Serializable]
  [ModelCheckMode(ModelCheckModes.CheckTableWhenFirstUse)]
  public class TestingCombGuid : TestingCombGuid<TestingCombGuid> { }

  /// <summary>测试表CombGuid</summary>
  public partial class TestingCombGuid<TEntity> : CommonCombGuidPKEntityBase<TEntity> where TEntity : TestingCombGuid<TEntity>, new()
  {
    #region 对象操作﻿
    private static readonly EntityCacheConfig.EntityCacheInfo _EntityCacheInfo;

    static TestingCombGuid()
    {
      // 用于引发基类的静态构造函数，所有层次的泛型实体类都应该有一个
      TEntity entity = new TEntity();

      _EntityCacheInfo = EntityCacheConfig.Current.Find("EmeTesting", "TestingCombGuid");

      var cache = Meta.Cache;
      if (_EntityCacheInfo.EntityCacheExpriod > 0) { cache.Expriod = _EntityCacheInfo.EntityCacheExpriod; }
      if (_EntityCacheInfo.EntityCacheMaxCount > 0) { cache.MaxCount = _EntityCacheInfo.EntityCacheMaxCount; }

      if (!_EntityCacheInfo.DisableSingleCache)
      {
        var singleCache = Meta.SingleCache;
        if (_EntityCacheInfo.SingleCacheExpriod > 0) { singleCache.Expriod = _EntityCacheInfo.SingleCacheExpriod; }
        singleCache.MaxCount = _EntityCacheInfo.SingleCacheMaxCount;
        if (_EntityCacheInfo.HoldSingleCache > -1) { singleCache.HoldCache = _EntityCacheInfo.HoldSingleCache > 0; }
        ...
        //        return id > 0 ? ec.Entities.Find(e => id == e.ID) : null;
        //    }
        //};
    ...
    //    var entity = new TestingCombGuid();
    //    entity.ID = nil;
    //    entity.Category = 0;
    //    entity.OrganizeID = 0;
    //    entity.RoleID = 0;
    ...
    public static EntityList<TEntity> FindAllByCreateUserID(Int32 createuserid)
    {
      ...
      if (session.Count > cache.MaxCount)
        return FindAll(__.CreateUserID, createuserid);
      else // 实体缓存
        return cache.Entities.FindAll(__.CreateUserID, createuserid);
    }
    ...
    /// <returns></returns>
    [DataObjectMethod(DataObjectMethodType.Select, false)]
    public static TEntity FindByID(CombGuid id)
    {
      var session = Meta.Session;
      var cache = session.Cache;
      if (session.Count > cache.MaxCount)
        ...
        return cache.Entities.Find(__.ID, id);
      // 单对象缓存
    ...
    public static EntityList<TEntity> FindAllByOrganizeID(Int32 organizeid)
    {
      ...
      if (session.Count > cache.MaxCount)
        ...
        return cache.Entities.FindAll(__.OrganizeID, organizeid);
    }
    ...
    public static TEntity FindByName(String name)
    {
      ...
      if (session.Count > cache.MaxCount)
        ...
        return cache.Entities.Find(__.Name, name);
      // 单对象缓存
    ...
    public static EntityList<TEntity> FindAllByRoleID(Int32 roleid)
    {
      ...
      if (session.Count > cache.MaxCount)
        ...
        return cache.Entities.FindAll(__.RoleID, roleid);
    }
    #endregion
