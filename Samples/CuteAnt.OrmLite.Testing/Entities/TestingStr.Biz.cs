...
namespace CuteAnt.OrmLite.Testing
{
  ...
  public class TestingStr : TestingStr<TestingStr> { }

  /// <summary>测试表String</summary>
  public partial class TestingStr<TEntity> : Entity<TEntity> where TEntity : TestingStr<TEntity>, new()
  {
    #region 对象操作﻿
    ...
    /// <returns></returns>
    [DataObjectMethod(DataObjectMethodType.Select, false)]
    public static TEntity FindByID(String id)
    {
      var session = Meta.Session;
      var cache = session.Cache;
      if (session.Count > cache.MaxCount)
