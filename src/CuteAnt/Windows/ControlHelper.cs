...
namespace System.Windows.Forms
{
	...
	public static class HmControlUtils
	{
		#region 在UI线程上执行委托
		...
		/// <param name="method"></param>
		/// <returns></returns>
		public static TResult Invoke<TResult>(this Control control, Action<TResult> method)
		{
		...
		//Int32 _pColor = 0;
		private static Color _Key = Color.FromArgb(255, 170, 0);

		private static Color _Num = Color.FromArgb(255, 58, 131);
		private static Color _KeyName = Color.FromArgb(0, 255, 255);
		...
		private static Regex _reg = new Regex(@"(?i)\b(0x|[0-9])([0-9a-fA-F\-]*)(.*?)\b", RegexOptions.Compiled);
		...
		private static Regex _reg2 = new Regex(@"(?i)(\b\w+\b)(\s*::\s*)(\b\w+\b)", RegexOptions.Compiled);
		...
		private static Regex _reg3 = new Regex(@"(?i)(\b\w+\b)(\s*[=:])[^:]\s*", RegexOptions.Compiled);
