	...
	public class ExtendData
	{
		...
		public T GetItem<T>(String key)
		{
			...
			else if (t.IsArray || value is IEnumerable)
			{
				...
				if (data == null) throw new HmExceptionBase("ExtendData无法分析数据" + value);
			...
			throw new HmExceptionBase("不支持的类型{0}，键{1}", typeof(T), key);
		}
		...
		public void SetItem(String key, Object value)
		{
			...
			throw new HmExceptionBase(String.Format("不支持的类型{0}，键{1}，数据{2}", t, key, value));
		}
		...
		public static ExtendData FromXml(String xml)
		{
			...
			{
				HmTrace.WriteInfo("Xml数据异常！" + ex.Message + Environment.NewLine + xml);
		...
		public String ToXml()
		{
			var doc = new XmlDocument();
			var rootName = Root;
			if (rootName.IsNullOrWhiteSpace()) rootName = "Extend";
