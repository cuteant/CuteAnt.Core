...
namespace CuteAnt.Xml
{
	...
	public static class XmlHelper
	{
		...
		public static void ToXml(this Object obj, Stream stream, Encoding encoding = null, String prefix = null, String ns = null, Boolean includeDeclaration = false, Boolean attachCommit = false)
		{
			...
			if (!type.IsPublic) throw new HmExceptionBase("类型{0}不是public，不能进行Xml序列化！", type.FullName);
		...
		internal static String XmlConvertToString(Object value)
		{
			...
			if (method == null) throw new HmExceptionBase("类型{0}不支持转为Xml字符串，请先用CanXmlConvert方法判断！", type);
		...
		{
			return (T)XmlConvertFromString(typeof(T), xml);
		}
		...
		internal static Object XmlConvertFromString(Type type, String xml)
		{
			...
			if (method == null) throw new HmExceptionBase("类型{0}不支持从Xml字符串转换，请先用CanXmlConvert方法判断！", type);
		...
		private static Boolean AttachCommitInternal(this XmlNode node, Type type)
		{
		...
		private static Boolean SetComment(this XmlNode node, MemberInfo member)
		{
			...
			var des = member.GetCustomAttributeX<DescriptionAttribute>(true);
			var dis = member.GetCustomAttributeX<DisplayNameAttribute>(true);
		...
		private static Boolean IsComment(this XmlNode node)
		{
			return node != null && node.NodeType == XmlNodeType.Comment;
		}
		...
		{
			if (rootName.IsNullOrWhiteSpace()) rootName = "xml";
