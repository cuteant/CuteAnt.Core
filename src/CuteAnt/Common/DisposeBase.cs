...
namespace CuteAnt
{
	...
	public abstract class DisposeBase : CriticalFinalizerObject, IDisposable2
	{
		...
		private void Dispose(Boolean disposing)
		{
			...
			{
				throw new HmExceptionBase("设计错误，OnDispose应该只被调用一次！代码不应该直接调用OnDispose，而应该调用Dispose。子类重载OnDispose时必须首先调用基类方法！");
			}
		}
		...
		protected virtual void OnDispose(Boolean disposing)
		{
			...
			{
				throw new HmExceptionBase("设计错误，OnDispose应该只被调用一次！代码不应该直接调用OnDispose，而应该调用Dispose。");
			}
			//if (disposing)
			//{
			//	// 释放托管资源
