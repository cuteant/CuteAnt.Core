	...
	public static class ControlHelper
	{
		...
		public static T FindControl<T>(Control control, String id) where T : Control
		{
			...
			while (queue.Count > 0)
			{
				...
				{
					// 没有指定控件ID，或者控件ID匹配
					//if (id.IsNullOrWhiteSpace() || String.Equals(control.ID, id, StringComparison.OrdinalIgnoreCase))
		...
		public static T FindControlUp<T>(Control control, String id) where T : Control
		{
			...
			while (queue.Count > 0)
			{
				...
				{
					// 没有指定控件ID，或者控件ID匹配
					if (id.IsNullOrWhiteSpace() || control.ID.EqualIgnoreCase(id))
					{
						return control as T;
					}
				}
		...
		{
			if (control == null) { return null; }
			if (eventName.IsNullOrWhiteSpace()) { return null; }
