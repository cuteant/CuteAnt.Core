...
namespace CuteAnt.IO
{
	...
	public abstract class StreamHandler : IStreamHandler
	{
		...
		private static Dictionary<String, LinkedList<IStreamHandler>> maps = new Dictionary<String, LinkedList<IStreamHandler>>();
		...
		private const String handlerKey = "CuteAnt.StreamHandler_";

		/// <summary>获取配置文件指定的处理器</summary>
		/// <returns></returns>
		private static Dictionary<String, List<Type>> GetHandler()
		{
		...
		private static void LoadConfig()
		{
			...
			{
				HmTrace.WriteException(ex, "从配置文件加载数据流处理器出错！");
			}
		}
