...
namespace CuteAnt.IO
{
	...
	public abstract class StreamClient
	{
		...
		public String StreamHandlerName
		{
			get
			{
				if (_StreamHandlerName == null && Uri != null)
				{
					...
					if (!Uri.AbsolutePath.IsNullOrWhiteSpace()) _StreamHandlerName = Path.GetFileNameWithoutExtension(Uri.AbsolutePath);
				}
		...
		protected virtual void Process(Stream stream)
		{
			...
			if (name.IsNullOrWhiteSpace()) throw new HmExceptionBase("未指定数据流总线名称StreamHandlerName！");
		...
		private class InternalStream : ReadWriteStream
		{
