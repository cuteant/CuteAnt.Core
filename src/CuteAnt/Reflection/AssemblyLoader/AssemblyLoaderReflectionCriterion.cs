...
{
  public class AssemblyLoaderReflectionCriterion : AssemblyLoaderCriterion
  {
    public delegate bool AssemblyPredicate(Assembly candidate, out IEnumerable<string> complaint);
    public delegate bool TypePredicate(Type candidate, out IEnumerable<string> complaint);

    /// <summary>Create a new criterion that filters assemblies by predicate. </summary>
    /// <param name="assemblyPredicate">A predicate which accepts an assembly an an argument.
    /// If this predicate returns true, the assembly will loaded and further inspection of the assembly with halt.
    /// If this predicate returns false, the predicate must provide a complaint explaining why the assembly does not meet the criterion described by the predicate.</param>
    /// <returns></returns>
    ...
    /// <summary>Create a new criterion that filters assemblies by predicate.</summary>
    /// <param name="typePredicate">A predicate which accepts a reflection-only type as an argument.
    /// If this predicate returns true, the assembly that provides the specified type will loaded and further inspection of the assembly with halt.
    /// If this predicate returns false, the predicate may provide a complaint explaining why the assembly does not meet the criterion described by the predicate.</param>
    /// <param name="defaultComplaints">If no predicate provides a complaint, then these default complaints are logged instead.</param>
    /// <returns></returns>
    /// <exception cref="System.ArgumentNullException">assemblyPredicate is null.</exception>
    public static AssemblyLoaderReflectionCriterion NewCriterion(TypePredicate typePredicate, IEnumerable<string> defaultComplaints)
    {
      ...
      return NewCriterion(
              (Assembly assembly, out IEnumerable<string> assemblyComplaints) =>
              {
                ...
                {
                  if (InterceptReflectionTypeLoadException(e, out assemblyComplaints)) { return false; }
                ...
                {
                  if (typePredicate(type, out IEnumerable<string> typeComplaints))
                  {
    ...
    private static AggregateException Flatten(ReflectionTypeLoadException rtle)
    {
    ...
    private AssemblyLoaderReflectionCriterion(AssemblyPredicate predicate)
      : base((object input, out IEnumerable<string> complaints) => predicate((Assembly)input, out complaints)) { }
    ...
    {
      var badFiles = new Dictionary<string, string>(StringComparer.Ordinal);
