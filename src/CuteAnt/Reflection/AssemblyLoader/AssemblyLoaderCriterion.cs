...
{
  /// <summary>A subsystem interested in loading additional assemblies passes an instance
  /// of AssemblyLoadCriterion to AssemblyLoader.QualifySearch to ensure that
  /// assemblies that meet a given criterion are loaded into memory.</summary>
  public abstract class AssemblyLoaderCriterion
  {
    /// <summary>An AssemblyLoadCriterion wraps a delegate where the predicate logic is implemented.</summary>
    ...
    /// <summary>AssemblyLoader invokes this wrapper for predicate when it needs to know whether an
    /// assembly is interesting to a subsystem that registered a criterion.</summary>
    ...
    {
      var isApproved = _predicate(input, out complaints);
      if (isApproved)
      {
        return true;
      }
