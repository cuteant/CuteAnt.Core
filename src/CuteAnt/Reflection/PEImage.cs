...
namespace CuteAnt.Reflection
{
  ...
  public class PEImage
  {
    ...
    public static PEImage Read(String file)
    {
      ...
      {
        if (s_logger.IsDebugLevelEnabled()) s_logger.LogDebug("PEImage.Read({0})出错 {1}", file, ex);
    ...
    private void ReadOptionalHeaders(BinaryReader reader)
    {
    ...
    private void ReadSections(BinaryReader reader, ushort count)
    {
      var stream = reader.BaseStream;
    ...
    private Boolean ReadCLIHeader(BinaryReader reader)
    {
      var stream = reader.BaseStream;
    ...
    private Boolean ReadMetadata(BinaryReader reader)
    {
      var stream = reader.BaseStream;
    ...
    private Int64 ResolveVirtualAddress(RVA rva)
    {
    ...
    private struct DataDirectory
    {
      ...
      public DataDirectory(BinaryReader reader)
        : this(reader.ReadUInt32(), reader.ReadUInt32())
      {
      }
    }
