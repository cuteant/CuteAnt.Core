...
namespace CuteAnt.Reflection
{
  ...
  public class TypeX : MemberInfoX
  {
    ...
    private delegate Object FastHandler(Object[] parameters);
    ...
    public override Object CreateInstance(params Object[] parameters)
    {
      if (Type.ContainsGenericParameters || Type.IsGenericTypeDefinition)
        throw new HmExceptionBase(Type.FullName + "类是泛型定义类，缺少泛型参数！");
    ...
    private FastHandler GetHandler(ConstructorInfo constructor)
    {
    ...
    private ConstructorInfo GetConstructor(Type[] paramTypes)
    {
    ...
    private static Type GetGenericType(String typeName, Boolean isLoadAssembly)
    {
      ...
      foreach (var item in pnames)
      {
        ...
        if (item.IsNullOrWhiteSpace()) { return gt; }
    ...
    private static Type GetArrayType(String typeName, Boolean isLoadAssembly)
    {
      ...
      if (pname.IsNullOrWhiteSpace()) return gt.MakeArrayType();
    ...
    /// <param name="paramTypes"></param>
    /// <returns></returns>
    public MethodInfoX GetMethod(String name, Type[] paramTypes)
    {
