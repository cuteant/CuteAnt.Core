...
namespace CuteAnt.Collections
{
	...
	public class ObjectPool<T> : DisposeBase //where T : new()
	{
		...
		public virtual void Push(T obj)
		{
			...
			if (stack.Count > Max)
			{
				if (_clearTimers == null) _clearTimers = new TimerX(ClearAboveMax, null, 0, HmTrace.Debug ? 10000 : 120000);
			}
			else
			{
				// 没到最大值，关闭定时器？
		...
		private TimerX _clearTimers;

		private void ClearAboveMax(Object state)
		{
			var stack = Stock;
