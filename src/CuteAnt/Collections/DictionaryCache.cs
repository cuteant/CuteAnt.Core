...
namespace CuteAnt.Collections
{
	...
	public class DictionaryCache<TKey, TValue> : /*DisposeBase, */IDictionary<TKey, TValue>, IDisposable
	{
		...
		public virtual TValue GetItem(TKey key, Func<TKey, TValue> func)
		{
			...
			if (items.TryGetValue(key, out item))
			{
				if (expriod <= 0 || !item.Expired) { return item.Value; }
			...
			else
			{
				...
				if (CacheDefault || !Object.Equals(value, _DefaultValue))
				{
					if (items.TryAdd(key, new CacheItem(value, expriod))) { StartTimer(); }
				}
				return value;
			}
		}
		...
		private void StartTimer()
		{
			...
			if (clearTimer == null)
			{
				...
				var timer = new TimerX(RemoveNotAlive, null, ClearExpriod * 1000, ClearExpriod * 1000);
				if (Interlocked.CompareExchange<TimerX>(ref clearTimer, timer, null) != null)
				{
					timer.Dispose();
					timer = null;
				}
		...
		{
			Items.TryAdd(key, new CacheItem(value, Expriod));
		}
		...
		public Boolean TryGetValue(TKey key, out TValue value)
		{
			...
			value = rs && item != null && (Expriod <= 0 || !item.Expired) ? item.Value : _DefaultValue;
			return rs;
		}
