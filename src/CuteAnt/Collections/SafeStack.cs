...
namespace CuteAnt.Collections
{
	...
	public class SafeStack<T> : DisposeBase, IStack<T>, IEnumerable<T>, ICollection, IEnumerable where T : class,ISafeStackItem
	{
		...
		{
			_array = new T[capacity]; lastNotFree = new Int32[capacity];
		}
		...
		public void Push(T item)
		{
			...
			for (Int32 i = 0; i < len; i++, len = _array.Length)
			{
		...
		private Object _lock_CheckSize = new Object();
		...
		private volatile Int32 lastNotFreeIndex = 0;

		//Int32[] lastFree = new Int32[SIZE_OF_CACHE];
		private Int32[] lastNotFree;// = new Int32[SIZE_OF_CACHE];

		private volatile Int32 LastFree = 0;
		...
		public void Clear()
		{
			...
			for (Int32 i = 0; i < _array.Length && i < len; i++) _array[i] = default(T);
		}
		...
		{
			for (Int32 i = 0; i < _array.Length && i < Count; i++) yield return _array[i];
		}
