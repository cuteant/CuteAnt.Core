	...
	public sealed class ThreadPoolX : IDisposable
	{
		...
		private SortedList<Int32, ThreadTask> Tasks
		{
			get
			{
				if (_Tasks == null)
				{
					_Tasks = new SortedList<Int32, ThreadTask>();
				}
				return _Tasks;
			}
		}
		...
		public static ThreadPoolX Create(String name)
		{
			...
			{
				throw new ArgumentNullException(name, "线程池名字不能为空！");
			}
		...
		public TaskState Abort(Int32 id)
		{
			...
			if (task != null && task.AbortMethod != null)
			{
				try
				{
					task.AbortMethod(task.Argument);
				}
				catch { }
			}
			return state;
		...
		public void AbortAllTask()
		{
			...
			foreach (var item in list)
			{
				if (item.AbortMethod != null)
				{
					try
					{
						item.AbortMethod(item.Argument);
					}
					catch { }
				}
			}
			...
			while (true)
			{
				try
				{
					...
					lock (SyncLock_Threads)
					{
						...
						//清理死线程
						for (Int32 i = Threads.Count - 1; i >= 0; i--)
						{
							if (Threads[i] == null)
							{
								Threads.RemoveAt(i);
								HmTrace.WriteException(Name + "线程池的线程对象为空，设计错误！");
							}
							else if (!Threads[i].IsAlive)
							{
								Threads[i].Dispose();
								HmTrace.WriteException(Threads[i].Name + "处于非活动状态，设计错误！");
								Threads.RemoveAt(i);
							}
						...
						{
							for (Int32 i = Threads.Count - 1; i >= 0 && count < 0; i--)
							{
		...
		private void thread_OnTaskFinished(object sender, EventArgs e)
		{
				...
				if (Threads.Contains(thread))
				{
					lock (SyncLock_Threads)
					{
						if (Threads.Contains(thread))
						{
							...
							HmTrace.WriteException("归还" + thread.Name + "时发现，线程被关闭了，设计错误！");
						}
					}
				}
				thread.Dispose();
		...
		{
			QueueUserWorkItem(callback, state, HmTrace.WriteDebug);
		}
		...
		/// <remarks>Code taken from SuperSocket Library
		/// &lt;a href="http://www.supersocket.net/"&gt;Kerry Jiang's SuperSocket Library&lt;/a&gt;
		/// </remarks>
		/// <returns></returns>
		public static Boolean ResetThreadPool(Int32? maxWorkingThreads, Int32? maxCompletionPortThreads, Int32? minWorkingThreads, Int32? minCompletionPortThreads)
		{
			if (maxWorkingThreads.HasValue || maxCompletionPortThreads.HasValue)
			{
				Int32 oldMaxWorkingThreads, oldMaxCompletionPortThreads;

				ThreadPool.GetMaxThreads(out oldMaxWorkingThreads, out oldMaxCompletionPortThreads);

				if (!maxWorkingThreads.HasValue) { maxWorkingThreads = oldMaxWorkingThreads; }

				if (!maxCompletionPortThreads.HasValue) { maxCompletionPortThreads = oldMaxCompletionPortThreads; }
			...
			{
				Int32 oldMinWorkingThreads, oldMinCompletionPortThreads;

				ThreadPool.GetMinThreads(out oldMinWorkingThreads, out oldMinCompletionPortThreads);

				if (!minWorkingThreads.HasValue) { minWorkingThreads = oldMinWorkingThreads; }

				if (!minCompletionPortThreads.HasValue) { minCompletionPortThreads = oldMinCompletionPortThreads; }
