...
{
  /// <summary>interface to convert linq expression to IL using dynamic method</summary>
  public interface ILinqToDynamicMethodConverter
  {
    /// <summary>try to create delegate using IL generation</summary>
  ...
  /// <summary>class to convert linq expression to IL using dynamic method</summary>
  public class LinqToDynamicMethodConverter : ILinqToDynamicMethodConverter
  {
    /// <summary>Implementation factory</summary>
    protected readonly ImplementationFactory ImplementationFactory;

    /// <summary>Default constructor</summary>
    /// <param name="implementationFactory"></param>
    public LinqToDynamicMethodConverter(ImplementationFactory implementationFactory) => ImplementationFactory = implementationFactory;
    ...
    public virtual bool TryCreateDelegate(IActivationExpressionResult expressionContext, ParameterExpression[] parameters, Expression[] extraExpressions, Expression finalExpression, out ActivationStrategyDelegate newDelegate)
    {
      ...
      try
      {
        ...
        {
          if (!ImplementationFactory.Locate<IConstantExpressionCollector>().GetConstantExpressions(expression, constants)) { return false; }
        }

        if (!ImplementationFactory.Locate<IConstantExpressionCollector>().GetConstantExpressions(finalExpression, constants)) { return false; }
        ...
        foreach (var expression in extraExpressions)
        {
          if (!TryGenerateIL(request, expression)) { return false; }
        }

        if (!TryGenerateIL(request, finalExpression)) { return false; }
    ...
    /// <summary>Try to create IL from linq expression, this is called recursively as the expression is walked</summary>
