...
{
  /// <summary>interface for generating IL for a constant expression</summary>
  public interface IConstantExpressionGenerator
  {
    /// <summary>Generate IL for a constant expression</summary>
  ...
  /// <summary>Class for converting constant expression to IL</summary>
  public class ConstantExpressionGenerator : IConstantExpressionGenerator
  {
    ...
    private bool ProcessConstantFromArrayTarget(DynamicMethodGenerationRequest request, ConstantExpression expression)
    {
      ...
      if (constantIndex < 0) { return false; }

#if NET40
      var field = request.Target.GetType().GetField("Items");
#else
...
#if NET40
      var field = request.Target.GetType().GetField("TArg" + constantIndex);
#else
