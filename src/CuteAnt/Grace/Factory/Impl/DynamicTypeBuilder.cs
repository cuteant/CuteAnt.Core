...
namespace Grace.Factory.Impl
{
    ...
    public class DynamicTypeBuilder
    {
...
#if NET40
        private static MethodInfo CloneMethod = typeof(IInjectionContext).GetMethod(nameof(IInjectionContext.Clone), EmptyArray<Type>.Instance);
        private static MethodInfo DelegateInvoke = typeof(ActivationStrategyDelegate).GetMethod(nameof(ActivationStrategyDelegate.Invoke),
        ...
        private static MethodInfo SetExtraDataMethod =
            typeof(IExtraDataContainer).GetMethod(nameof(IExtraDataContainer.SetExtraData), new[] { typeof(object), typeof(object), typeof(bool) });
#else
        private static MethodInfo CloneMethod = typeof(IInjectionContext).GetRuntimeMethod(nameof(IInjectionContext.Clone), EmptyArray<Type>.Instance);
