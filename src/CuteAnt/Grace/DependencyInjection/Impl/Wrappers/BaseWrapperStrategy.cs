...
{
  /// <summary>Base strategy for wrappers to inherit from</summary>
  public abstract class BaseWrapperStrategy : ConfigurableActivationStrategy, ICompiledWrapperStrategy
  {
    ...
    internal const string CreateFuncMethodName = "CreateFunc";
    internal const string CreateLazyMethodName = "CreateLazy";

    /// <summary>Activation delegates</summary>
    ...
    protected BaseWrapperStrategy(Type activationType, IInjectionScope injectionScope)
    ...
    public static IKnownValueExpression CreateKnownValueExpression(IActivationExpressionRequest request,
      Type argType, string valueId, string hintName = null, int? position = null)
    {
      const string _getExtraDataMethodName = nameof(IExtraDataContainer.GetExtraData);
      var getMethod = typeof(IExtraDataContainer).GetRuntimeMethod(_getExtraDataMethodName, new[] { typeof(object) });
