...
{
  /// <summary>holds a set of activation strategies</summary>
  ...
  public class ActivationStrategyCollection<T> : IActivationStrategyCollection<T> where T : class, IActivationStrategy
  {
    ...
    public ActivationStrategyCollection(Type exportType) => _exportType = exportType;

    /// <summary>Add a new strategy to collection</summary>
    ...
    public void AddStrategy(T strategy, object key)
    {
      if (key == null)
      {
        if (_hasPriorities || strategy.Priority != 0)
        {
          ...
          {
            if (strategy.Priority > _strategies[index].Priority) { break; }
          }
    ...
    /// <summary>Strategies that are non keyed</summary>
    /// <returns></returns>
    public ImmutableArray<T> GetStrategies() => _strategies;

    /// <summary>Get primary strategy</summary>
    /// <returns></returns>
    public T GetPrimary() => _primary;

    /// <summary>list of strategies and their keys</summary>
    /// <returns></returns>
    public IEnumerable<KeyValuePair<object, T>> GetKeyedStrategies() => _keyedStrategies;

    /// <summary>Get a keyed strategy</summary>
    /// <param name="key">key</param>
    /// <returns></returns>
    public T GetKeyedStrategy(object key) => _keyedStrategies.GetValueOrDefault(key);

    /// <summary>Clone the collection</summary>
    /// <returns></returns>
    ...
    /// <summary>Dispose of collection</summary>
    public void Dispose()
    {
