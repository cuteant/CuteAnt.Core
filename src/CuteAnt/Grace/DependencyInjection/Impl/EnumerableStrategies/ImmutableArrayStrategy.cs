...
{
  /// <summary>Strategy for creating ImmutableArray</summary>
  public class ImmutableArrayStrategy : BaseGenericEnumerableStrategy
  {
    /// <summary>Default constructor</summary>
    /// <param name="injectionScope"></param>
    public ImmutableArrayStrategy(IInjectionScope injectionScope)
      : base(typeof(ImmutableArray<>), injectionScope) { }
    ...
    public override IActivationExpressionResult GetActivationExpression(IInjectionScope scope, IActivationExpressionRequest request)
    {
      ...
      const string _fromMethodName = nameof(ImmutableArray.From);
      var fromMethod = typeof(ImmutableArray).GetRuntimeMethods().First(m => string.Equals(_fromMethodName, m.Name, StringComparison.Ordinal) && m.GetParameters().Length == 2);
