...
{
  /// <summary>Base class for all delegate based strategies (i.e. FactoryOneArgStrategy etc)</summary>
  ...
  public class DelegateBaseExportStrategy<T> : BaseInstanceExportStrategy
  {
    ...
    /// <returns></returns>
    protected override IActivationExpressionResult CreateExpression(
      IInjectionScope scope, IActivationExpressionRequest request, ICompiledLifestyle lifestyle)
    {
      if (lifestyle == null) { return CreateExpression(scope, request); }

      return lifestyle.ProvideLifestyleExpression(scope, request,
          expressionRequest => CreateExpression(scope, expressionRequest));
    }

    /// <summary>Create expression that calls a delegate</summary>
    /// <param name="scope">scope for the request</param>
    /// <param name="request">activation request</param>
    /// <returns></returns>
    protected virtual IActivationExpressionResult CreateExpression(IInjectionScope scope, IActivationExpressionRequest request)
        => ExpressionUtilities.CreateExpressionForDelegate(DelegateInstance, ShouldTrackDisposable(scope), scope, request, this);
