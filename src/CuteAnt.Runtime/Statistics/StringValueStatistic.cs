...
{
  public class StringValueStatistic : ICounter<string>
  {
    ...
    public static StringValueStatistic FindOrCreate(StatisticName name, Func<string> f, CounterStorage storage = CounterStorage.LogOnly)
    {
      lock (lockable)
      {
        if (registeredStatistics.TryGetValue(name.Name, out StringValueStatistic stat))
        {
          return stat;
        }
    ...
    public static void Delete(string name)
    {
      lock (lockable)
      {
        if (registeredStatistics.TryGetValue(name, out StringValueStatistic stat))
        {
