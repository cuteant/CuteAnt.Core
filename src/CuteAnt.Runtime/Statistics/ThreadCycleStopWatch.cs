...
{
  /// <summary>Stopwatch for CPU time of a thread. 
  ...
  /// Otherwise, QueryThreadCycleTime returns undefined (garbage) results. </summary>
  [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1049:TypesThatOwnNativeResourcesShouldBeDisposable")]
  public class TimeIntervalThreadCycleCounterBased : ITimeInterval
  {
    private readonly double cyclesPerSecond;
    private static readonly ILogger s_logger = TraceLogger.GetLogger("CuteAnt.RunTime.ThreadCycleCounter");//, TraceLogger.LoggerType.Runtime);
    ...
    /// <summary>Obtain current time of stopwatch since last Stop method. You may call this from any thread.</summary>
    public TimeSpan Elapsed
    {
      get { return TimeSpan.FromSeconds(((double)elapsedCycles) / (cyclesPerSecond)); }
    }

    /// <summary>Create thread CPU timing object. You may call this from a thread outside the one you wish to measure.</summary>
    ...
    /// <summary>Start measuring time thread is using CPU. Must invoke from thread to be measured!</summary>
    public void Start()
    {
    ...
    /// <summary>Restart measuring time thread is using CPU. Must invoke from thread to be measured! </summary>
    public void Restart()
    {
      elapsedCycles = 0;
      Start();
    ...
    /// <summary>Stop measuring time thread is using CPU. Must invoke from thread to be measured! </summary>
    public void Stop()
    {
      if (!running) return;
      ...
      {
        //var log = TraceLogger.GetLogger("CuteAnt.RunTime.Thread Cycle Counter");//, TraceLogger.LoggerType.Runtime);
        if (s_logger.IsTraceLevelEnabled()) s_logger.LogTrace(0, "Invalid cycle counts startCycles = {0}, stopCycles = {1}", startCycles, stopCycles);
