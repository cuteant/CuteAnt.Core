...
namespace CuteAnt.Runtime
{
  ...
  /// <summary>A detailed statistic counter. Usually a low level performance statistic used in troubleshooting scenarios.</summary>
  public interface ICounter
  {
    /// <summary>the name of the statistic counter</summary>
    string Name { get; }

    /// <summary>if this the statistic counter value is delta since last value reported or an absolute value</summary>
    ...
    //void TrackMetric(ILogger logger);
  }
  ...
  public class CounterStatistic : ICounter<long>
  {
    ...
    [ThreadStatic]
    private static bool isManagedThread;
    ...
    private long nonThreadsCounter; // one for all non-Orleans threads
    ...
    public static void SetManagedThread()
    {
      if (!isManagedThread)
      {
        lock (lockable)
    ...
    private static CounterStatistic FindOrCreate_Impl(StatisticName name, bool useDelta, CounterStorage storage, bool isHidden)
    {
      lock (lockable)
      {
        if (registeredStatistics.TryGetValue(name.Name, out CounterStatistic stat))
        {
          return stat;
        }
    ...
    // The managed threads aggregate stats in per thread local storage list.
    // For non managed threads (.NET IO completion port threads, thread pool timer threads) we don't want to allocate a thread local storage,
    // since we don't control how many of those threads are created (could lead to too much thread local storage allocated).
    // Thus, for non managed threads, we use a counter shared between all those threads and Interlocked.Add it (creating small contention).
    public void IncrementBy(long n)
    {
      if (isManagedThread)
      {
        ...
        var refLong = allStatisticsFromSpecificThread[id];

        if (refLong == null)
        {
          refLong = new ReferenceLong();
          allStatisticsFromSpecificThread[id] = refLong;
          specificStatisticFromAllThreads.Push(refLong);
        }
      ...
      else
      {
        if (n == 1)
        {
          Interlocked.Increment(ref nonThreadsCounter);
        }
        else
        {
          Interlocked.Add(ref nonThreadsCounter, n);
        }
      }
    }
    ...
    public long GetCurrentValue()
    {
      ...
      return val + Interlocked.Read(ref nonThreadsCounter);
    }
    ...
    {
      long current = GetCurrentValueAndDelta(out long delta);
    ...
    //public void TrackMetric(ILogger logger)
    //{
