...
// WCF - System.ServiceModel.Internals\System\Runtime\TypeHelper.cs
//-----------------------------------------------------------------------------
...
namespace CuteAnt.Reflection
{
  public static class TypeHelper
  {
    ...
    public static readonly Type ObjectType = TypeConstants.Object; // typeof(object);
    public static readonly Type StringType = TypeConstants.String; // typeof(string);
    public static readonly Type TypeType = TypeConstants.Type; // typeof(Type);
    ...
    public static T Convert<T>(object source)
    {
      ...
      if (source == null)
      {
        if (conversionType.IsValueType && !IsNullableType(conversionType))
        {
          throw Fx.Exception.AsError(new InvalidCastException(InternalSR.CannotConvertObject(source, conversionType)));
        }
      ...
      if (conversionType == GuidType)
      {
        return (T)(object)source.ToGuid();
      }
      else if (conversionType == CombGuidType)
      {
        CombGuid comb;
        if (CombGuid.TryParse(source, CombGuidSequentialSegmentType.Comb, out comb)) { return (T)(object)comb; }
        if (CombGuid.TryParse(source, CombGuidSequentialSegmentType.Guid, out comb)) { return (T)(object)comb; }
        return (T)(object)CombGuid.Null;
      }
      ...
      //  }
      //  return (T)System.Convert.ChangeType(source, conversionType);
      //}
      #endregion
    ...
    /// <summary>Instantiate a type with constructor arguments provided directly and/or from <see cref="ObjectContainer"/>.</summary>
    /// <param name="instanceType">The type to activate</param>
    /// <param name="parameters">Constructor arguments not provided by the <see cref="ObjectContainer"/>.</param>
    /// <returns>An activated object of type instanceType</returns>
    public static object CreateInstanceWithParameters(this Type instanceType, params object[] parameters) =>
        ObjectContainer.CreateInstance(instanceType, parameters);

    /// <summary>Instantiate a type with constructor arguments provided directly and/or from <see cref="ObjectContainer"/>.</summary>
    /// <typeparam name="T">The type to activate</typeparam>
    /// <param name="parameters">Constructor arguments not provided by the <see cref="ObjectContainer"/>.</param>
    /// <returns>An activated object of type T</returns>
    public static object CreateInstanceWithParameters<T>(params object[] parameters) =>
        ObjectContainer.CreateInstance<T>(parameters);

    #endregion

    #region **& class TypeMeta<T> &**
    ...
    private static class TypeMeta<T>
    {
      ...
      static TypeMeta()
      {
        EmptyCtorFn = Reflect.GetConstructorMethodToCache(typeof(T));
      }
    }
