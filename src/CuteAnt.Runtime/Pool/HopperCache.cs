...
// System.ServiceModel.Internals\System\Runtime\Collections\HopperCache.cs
//------------------------------------------------------------
...
{
  /// <summary>This cache works like a MruCache, but operates loosely and without locks in the mainline path.</summary>
  /// <remarks>
  ...
  {
    private readonly int m_hopperSize;
    private readonly bool m_weak;

    private Hashtable m_outstandingHopper;
    private Hashtable m_strongHopper;
    private Hashtable m_limitedHopper;
    private int m_promoting;
    private LastHolder m_mruEntry;

    public HopperCache(int hopperSize, bool weak)
    {
      Fx.Assert(hopperSize > 0, "HopperCache hopperSize must be positive.");

      m_hopperSize = hopperSize;
      m_weak = weak;

      m_outstandingHopper = new Hashtable(hopperSize * 2);
      m_strongHopper = new Hashtable(hopperSize * 2);
      m_limitedHopper = new Hashtable(hopperSize * 2);
    }
    ...
    public void Add(object key, object value)
    {
      ...
      if (m_weak && !object.ReferenceEquals(value, DBNull.Value))
      {
        value = new WeakReference(value);
      }

      Fx.Assert(m_strongHopper.Count <= m_hopperSize * 2,
          "HopperCache strongHopper is bigger than it's allowed to get.");

      if (m_strongHopper.Count >= m_hopperSize * 2)
      {
        Hashtable recycled = m_limitedHopper;
        recycled.Clear();
        ...
        {
          m_limitedHopper = m_strongHopper;
          m_strongHopper = recycled;
        }
      }
      else
      {
    ...
    public object GetValue(object syncObject, object key)
    {
      ...
      LastHolder last = m_mruEntry;
      if (last != null && key.Equals(last.Key))
      {
        if (m_weak && (weakRef = last.Value as WeakReference) != null)
        {
      ...
      object origValue = m_outstandingHopper[key];
      value = m_weak && (weakRef = origValue as WeakReference) != null ? weakRef.Target : origValue;
      if (value != null)
      {
        m_mruEntry = new LastHolder(key, origValue);
        return value;
      }

      // Try the subsequent hoppers.
      origValue = m_strongHopper[key];
      value = m_weak && (weakRef = origValue as WeakReference) != null ? weakRef.Target : origValue;
      if (value == null)
      {
        origValue = m_limitedHopper[key];
        value = m_weak && (weakRef = origValue as WeakReference) != null ? weakRef.Target : origValue;
        if (value == null)
        {
      ...
      try
      {
        ...
        finally
        {
          ...
          wasPromoting = Interlocked.CompareExchange(ref m_promoting, 1, 0);
        }

        // Only one thread can be inside this 'if' at a time.
        if (wasPromoting == 0)
        {
          Fx.Assert(m_outstandingHopper.Count <= m_hopperSize,
              "HopperCache outstandingHopper is bigger than it's allowed to get.");

          if (m_outstandingHopper.Count >= m_hopperSize)
          {
            lock (syncObject)
            {
              Hashtable recycled = m_limitedHopper;
              ...
              {
                m_limitedHopper = m_strongHopper;
                m_strongHopper = m_outstandingHopper;
                m_outstandingHopper = recycled;
              }
            }
          }
          else
          {
            ...
            m_outstandingHopper[key] = origValue;
          }
        }
      }
      finally
    ...
    private class LastHolder
    {
      ...
      internal object Key { get { return m_key; } }
