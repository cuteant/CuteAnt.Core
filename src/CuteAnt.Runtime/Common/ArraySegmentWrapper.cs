...
namespace CuteAnt
{
  // Note: users should make sure they copy the fields out of an ArraySegmentWrapper onto their stack
  ...
  // three fields from an ArraySegmentWrapper may not see the same ArraySegmentWrapper from one call to another
  // (ie, users could assign a new value to the old location).
  //[Serializable]
  // After .NET 4.5 RTMs, we can undo this and expose the full surface area to CoreCLR
  public struct ArraySegmentWrapper<T> : IList<T>, IReadOnlyList<T>
  {
    public static readonly ArraySegmentWrapper<T> Empty = new ArraySegmentWrapper<T>(EmptyArray<T>.Instance, false);
    ...
    public ArraySegmentWrapper(T[] array, Boolean canReturn)
    {
      //if (array == null)
      //  throw new ArgumentNullException("array");
      //Contract.EndContractBlock();

      _segment = new ArraySegment<T>(array);
      _array = array;
      _offset = 0;
      _count = array.Length;
    ...
    public ArraySegmentWrapper(T[] array, int offset, int count, Boolean canReturn)
    {
      //if (array == null)
      //  throw new ArgumentNullException("array");
      //if (offset < 0)
    ...
    public ArraySegmentWrapper(ArraySegment<T> segment, Boolean canReturn)
    {
      _segment = segment;
    ...
    public ArraySegment<T> Segment { get { return _segment; } }
    ...
    public T[] Array
    {
      get
      {
        Contract.Assert((null == _array && 0 == _offset && 0 == _count)
                        ...
                        "ArraySegmentWrapper is invalid");
    ...
    public int Offset
    {
      get
      {
        ...
        // after reading each field out of an ArraySegmentWrapper into their stack.
    ...
    {
      if (obj is ArraySegmentWrapper<T>)
        return Equals((ArraySegmentWrapper<T>)obj);
      else
        return false;
    }

    public bool Equals(ArraySegmentWrapper<T> obj)
    {
      return obj._array == _array && obj._offset == _offset && obj._count == _count && obj._canReturn == _canReturn;
    }

    public static bool operator ==(ArraySegmentWrapper<T> a, ArraySegmentWrapper<T> b)
    {
      return a.Equals(b);
    }
    ...
    public static implicit operator ArraySegmentWrapper<T>(ArraySegment<T> x)
    {
      return new ArraySegmentWrapper<T>(x);
    }
    public static implicit operator ArraySegment<T>(ArraySegmentWrapper<T> x)
    {
      return x.Segment;
    }
    ...
    private sealed class ArraySegmentEnumerator : IEnumerator<T>
    {
      ...
      internal ArraySegmentEnumerator(ArraySegmentWrapper<T> arraySegment)
      {
