    ...
    public class BufferedOutputStream : Stream
    {
        ...
        public override void Write(ReadOnlySpan<byte> buffer)
        {
            ...
            if (size > remainingSizeInChunk)
            {
                ...
                {
                    buffer.Slice(0, remainingSizeInChunk).CopyTo(new Span<byte>(_currentChunk, _currentChunkSize, remainingSizeInChunk));
            ...
            if (offset > 0)
            {
                buffer.Slice(offset).CopyTo(new Span<byte>(_currentChunk, _currentChunkSize, size));
            }
            else
            {
                buffer.CopyTo(new Span<byte>(_currentChunk, _currentChunkSize, size));
            }
        ...
        /// <summary> Write a <c>Guid</c> value to the stream. </summary>
        public void WriteValue(in CombGuid id)
        {
