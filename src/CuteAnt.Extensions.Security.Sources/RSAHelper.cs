...
namespace CuteAnt.Security
{
	...
	internal static class RSAHelper
	{
		...
		/// <returns></returns>
		internal static String[] GenerateKey(int keySize = 1024)
		{
		...
		/// <returns></returns>
		internal static Byte[] Encrypt(Byte[] buf, String pubKey)
		{
		...
		/// <returns></returns>
		internal static Byte[] Decrypt(Byte[] buf, String priKey)
		{
			var rsa = new RSACryptoServiceProvider();
		...
		/// <param name="pubKey"></param>
		/// <returns></returns>
		internal static Byte[] EncryptWithDES(Byte[] buf, String pubKey)
		{
			return Encrypt<DESCryptoServiceProvider>(buf, pubKey);
		}
		...
		/// <param name="priKey"></param>
		/// <returns></returns>
		internal static Byte[] DecryptWithDES(Byte[] buf, String priKey)
		{
			return Decrypt<DESCryptoServiceProvider>(buf, priKey);
		}
		...
		/// <param name="pubKey"></param>
		/// <returns></returns>
		internal static Byte[] Encrypt<TSymmetricAlgorithm>(Byte[] buf, String pubKey) where TSymmetricAlgorithm : SymmetricAlgorithm, new()
		{
			// 随机产生对称加密密钥
		...
		/// <param name="priKey"></param>
		/// <returns></returns>
		internal static Byte[] Decrypt<TSymmetricAlgorithm>(Byte[] buf, String priKey) where TSymmetricAlgorithm : SymmetricAlgorithm, new()
		{
		...
		/// <param name="priKey"></param>
		/// <returns></returns>
		internal static Byte[] Sign(Byte[] buf, String priKey)
		{
			var rsa = new RSACryptoServiceProvider();
			rsa.FromXmlString(priKey);
		...
		/// <returns></returns>
		internal static Boolean Verify(Byte[] buf, String pukKey, Byte[] rgbSignature)
		{
			var rsa = new RSACryptoServiceProvider();
		...
		/// <param name="buf"></param>
		/// <returns></returns>
		internal static Byte[] SetRandom(this Byte[] buf)
		{
