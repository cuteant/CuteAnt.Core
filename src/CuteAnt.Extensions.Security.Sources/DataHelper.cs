...
namespace CuteAnt.Security
{
	...
	internal static class DataHelper
	{
		...
		internal static String Hash(String str)
		{
			...
			//var by = md5.ComputeHash(StringHelper.UTF8NoBOM.GetBytes(str));
			//return BitConverter.ToString(by).Replace("-", "");
			return str.MD5().ToHex();
		}
		...
		//	key = Hash(key);
		//	String str = key;
		//	Byte[] bts = StringHelper.UTF8NoBOM.GetBytes(str);
		...
		//	bts = StringHelper.UTF8NoBOM.GetBytes(str);
		//	//倒序
		//	Array.Reverse(bts);
		//	if (bts.Length != max) Array.Resize<Byte>(ref bts, max);
		//	sa.IV = bts;
		...
		//{
		//	if (content.IsNullOrWhiteSpace()) throw new ArgumentNullException("content");
		//	var data = StringHelper.UTF8NoBOM.GetBytes(content);
		...
		//	if (key.IsNullOrWhiteSpace()) throw new ArgumentNullException("key");
		...
		///// <param name="key">密码字符串经MD5散列后作为DES密码</param>
		///// <returns></returns>
		//internal static String Descrypt(String content, String key)
		//{
		//	if (content.IsNullOrWhiteSpace()) throw new ArgumentNullException("content");
		...
		//	return StringHelper.UTF8NoBOM.GetString(data);
		//}
		...
		///// <param name="key">密码字符串经MD5散列后作为DES密码</param>
		///// <returns></returns>
		//internal static Byte[] Descrypt(Byte[] data, String key)
		//{
		//	if (data == null || data.Length < 1) throw new ArgumentNullException("data");
		//	if (key.IsNullOrWhiteSpace()) throw new ArgumentNullException("key");
		...
		/// <returns></returns>
		internal static Byte[] RC4(Byte[] data, String pass)
		{
			if (data == null || pass == null) return null;
			Byte[] output = new Byte[data.Length];
			Int64 i = 0;
			Int64 j = 0;
			Byte[] mBox = GetKey(StringHelper.UTF8NoBOM.GetBytes(pass), 256);
		...
		{
			if (data == null | priKey.IsNullOrWhiteSpace()) return null;
		...
		internal static Boolean Verify(Byte[] data, String signdata, String pubKey)
		{
			if (data == null ||
					data.Length < 1 ||
					signdata.IsNullOrWhiteSpace() ||
					pubKey.IsNullOrWhiteSpace()) return false;
		...
		//internal static String ToHex(this Byte[] data, Int32 offset = 0, Int32 count = 0)
		//{
		//	return IOHelper.ToHex(data, offset, count);
		//}
		...
		[Obsolete("=>IOHelper.ToHex")]
		[EditorBrowsable(EditorBrowsableState.Never)]
		internal static Byte[] FromHex(String data, Int32 startIndex = 0, Int32 length = 0)
		{
			return IOHelper.ToHex(data, startIndex, length);
		}
