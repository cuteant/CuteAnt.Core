...
namespace CuteAnt.AsyncEx
{
  public sealed partial class AsyncContext
  {
    ...
    private sealed class TaskQueue : IDisposable
    {
      ...
      /// <summary>Gets a blocking enumerable that removes items from the queue. This enumerable only completes after <see cref="CompleteAdding"/> has been called.</summary>
      /// <returns>A blocking enumerable that removes items from the queue.</returns>
      public IEnumerable<Tuple<Task, bool>> GetConsumingEnumerable() => _queue.GetConsumingEnumerable();
      ...
      /// <summary>Attempts to add the item to the queue. If the queue has been marked as complete for adding, this method returns <c>false</c>.</summary>
      ...
      /// <summary>Marks the queue as complete for adding, allowing the enumerator returned from <see cref="GetConsumingEnumerable"/> to eventually complete. This method may be called several times.</summary>
