    ...
    public void TraceEtwException(Exception exception, TraceEventType eventType)
    {
      switch (eventType)
      {
        ...
        case TraceEventType.Warning:
          ...
          {
            TraceCore.ThrowingEtwException(_diagnosticTrace, _eventSourceName, exception != null ? exception.ToString() : string.Empty, exception);
          }
          break;
        case TraceEventType.Critical:
          if (TraceCore.UnhandledExceptionIsEnabled(_diagnosticTrace))
          {
            TraceCore.EtwUnhandledException(_diagnosticTrace, exception != null ? exception.ToString() : string.Empty, exception);
          }
          break;
        default:
          if (TraceCore.ThrowingExceptionVerboseIsEnabled(_diagnosticTrace))
          {
            TraceCore.ThrowingEtwExceptionVerbose(_diagnosticTrace, _eventSourceName, exception != null ? exception.ToString() : string.Empty, exception);
          }
          break;
      }
    }
...
#pragma warning disable 618
      logger = new EventLogger(_eventSourceName, _diagnosticTrace);
    ...
    internal void TraceFailFast(string message, EventLogger logger)
    {
      if (logger != null)
      {
        try
        {
          ...
          finally
          {
            logger.LogEvent(TraceEventType.Critical,
                ...
                (uint)CuteAnt.Diagnostics.EventLogEventId.FailFast,
                message,
                stackTrace);
          }
        }
        catch (Exception ex)
        {
          logger.LogEvent(TraceEventType.Critical,
              FailFastEventLogCategory,
              (uint)CuteAnt.Diagnostics.EventLogEventId.FailFastException,
              ex.ToString());
          if (Fx.IsFatal(ex))
          {
            throw;
