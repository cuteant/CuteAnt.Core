...
namespace CuteAnt.AsyncEx
{
  ...
  public sealed class AsyncCollection<T>
  {
    ...
    /// <summary>A condition variable that is signalled when the collection is completed or not full.</summary>
    ...
    /// <summary>Whether the collection has been marked completed for adding.</summary>
    ...
    internal async Task DoAddAsync(T item, CancellationToken cancellationToken, bool sync)
    {
      using (sync ? _mutex.Lock() : await _mutex.LockAsync().ConfigureAwait(false))
      {
        ...
        {
          if (sync) { _completedOrNotFull.Wait(cancellationToken); }
          else { await _completedOrNotFull.WaitAsync(cancellationToken).ConfigureAwait(false); }
        }

        // If the queue has been marked complete, then abort.
        if (_completed) { throw new InvalidOperationException("Add failed; the producer/consumer collection has completed adding."); }

        if (!_collection.TryAdd(item)) { throw new InvalidOperationException("Add failed; the add to the underlying collection failed."); }

        _completedOrNotEmpty.Notify();
      }
    }

    /// <summary>Adds an item to the producer/consumer collection. Throws <see cref="InvalidOperationException"/> 
    /// if the producer/consumer collection has completed adding or if the item was rejected by the underlying collection.</summary>
    /// <param name="item">The item to add.</param>
    /// <param name="cancellationToken">A cancellation token that can be used to abort the add operation.</param>
    public Task AddAsync(T item, CancellationToken cancellationToken) => DoAddAsync(item, cancellationToken, sync: false);

    /// <summary>Adds an item to the producer/consumer collection. Throws <see cref="InvalidOperationException"/> 
    /// if the producer/consumer collection has completed adding or if the item was rejected by the underlying collection. This method may block the calling thread.</summary>
    /// <param name="item">The item to add.</param>
    /// <param name="cancellationToken">A cancellation token that can be used to abort the add operation.</param>
    ...
    /// <summary>Waits until an item is available to take. Returns <c>false</c> if the producer/consumer collection has completed adding and there are no more items.</summary>
    ...
    private async Task<bool> DoOutputAvailableAsync(CancellationToken cancellationToken, bool sync)
    {
      using (sync ? _mutex.Lock() : await _mutex.LockAsync().ConfigureAwait(false))
      {
        ...
        {
          if (sync) { _completedOrNotEmpty.Wait(cancellationToken); }
          else { await _completedOrNotEmpty.WaitAsync(cancellationToken).ConfigureAwait(false); }
        }
        return !Empty;
      }
    }

    /// <summary>Asynchronously waits until an item is available to take. Returns <c>false</c> if the producer/consumer collection has completed adding and there are no more items.</summary>
    ...
    /// <summary>Synchronously waits until an item is available to take. Returns <c>false</c> if the producer/consumer collection has completed adding and there are no more items.</summary>
    /// <param name="cancellationToken">A cancellation token that can be used to abort the wait.</param>
    ...
    private async Task<T> DoTakeAsync(CancellationToken cancellationToken, bool sync)
    {
      using (sync ? _mutex.Lock() : await _mutex.LockAsync().ConfigureAwait(false))
      {
        ...
        if (_completed && Empty) { throw new InvalidOperationException("Take failed; the producer/consumer collection has completed adding and is empty."); }

        if (!_collection.TryTake(out T item)) { throw new InvalidOperationException("Take failed; the take from the underlying collection failed."); }
    ...
    /// <summary>Takes an item from the producer/consumer collection. Returns the item. Throws <see cref="InvalidOperationException"/> 
    /// if the producer/consumer collection has completed adding and is empty, or if the take from the underlying collection failed.</summary>
    /// <param name="cancellationToken">A cancellation token that can be used to abort the take operation.</param>
    public Task<T> TakeAsync(CancellationToken cancellationToken) => DoTakeAsync(cancellationToken, sync: false);

    /// <summary>Takes an item from the producer/consumer collection. Returns the item. Throws <see cref="InvalidOperationException"/> 
    /// if the producer/consumer collection has completed adding and is empty, or if the take from the underlying collection failed. This method may block the calling thread.</summary>
    /// <param name="cancellationToken">A cancellation token that can be used to abort the take operation.</param>
