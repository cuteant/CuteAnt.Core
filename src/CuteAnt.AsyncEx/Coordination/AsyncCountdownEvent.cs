...
namespace CuteAnt.AsyncEx
{
  ...
  {
    /// <summary>The underlying manual-reset event.</summary>
    ...
    /// <summary>Gets the current number of remaining signals before this event becomes set. This member is seldom used; code using this member has a high possibility of race conditions.</summary>
    public long CurrentCount
    {
      get { lock (_mre) { return _count; } }
    }

    /// <summary>Asynchronously waits for the count to reach zero.</summary>
    public Task WaitAsync() => _mre.WaitAsync();

    /// <summary>Synchronously waits for the count to reach zero. This method may block the calling thread.</summary>
    ...
    /// <summary>Attempts to modify the current count by the specified amount.</summary>
    ...
    /// <summary>Adds the specified value to the current count.</summary>
    /// <param name="addCount">The amount to change the current count.</param>
    public void AddCount(long addCount) => ModifyCount(addCount, true);

    /// <summary>Adds one to the current count.</summary>
    public void AddCount() => AddCount(1);

    /// <summary>Subtracts the specified value from the current count.</summary>
    /// <param name="signalCount">The amount to change the current count.</param>
    public void Signal(long signalCount) => ModifyCount(signalCount, false);

    /// <summary>Subtracts one from the current count.</summary>
