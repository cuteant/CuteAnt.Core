...
{
  /// <summary>A collection of cancelable <see cref="TaskCompletionSource{T}"/> instances. Implementations must assume the caller is holding a lock.</summary>
  ...
  public interface IAsyncWaitQueue<T>
  {
    ...
    /// <summary>Creates a new entry and queues it to this wait queue. The returned task must support both synchronous and asynchronous waits.</summary>
    /// <returns>The queued task.</returns>
    Task<T> Enqueue();

    /// <summary>Removes a single entry in the wait queue and completes it. This method may only be called if <see cref="IsEmpty"/> is <c>false</c>. The task continuations for the completed task must be executed asynchronously.</summary>
    ...
    /// <summary>Removes all entries in the wait queue and completes them. The task continuations for the completed tasks must be executed asynchronously.</summary>
    ...
    ///// <summary>Attempts to remove an entry from the wait queue. Returns a disposable that cancels the entry.</summary>
    ///// <param name="task">The task to cancel.</param>
    ///// <returns>A value indicating whether the entry was found and canceled.</returns>
    //IDisposable TryCancel(Task task);

    /// <summary>Attempts to remove an entry from the wait queue and cancels it. The task continuations for the completed task must be executed asynchronously.</summary>
    /// <param name="task">The task to cancel.</param>
    ...
    /// <summary>Removes all entries from the wait queue and cancels them. The task continuations for the completed tasks must be executed asynchronously.</summary>
  ...
  /// <summary>Provides extension methods for wait queues.</summary>
  public static class AsyncWaitQueueExtensions
  {
    /// <summary>Creates a new entry and queues it to this wait queue. If the cancellation token is already canceled, this method immediately returns a canceled task without modifying the wait queue.</summary>
  ...
  /// <summary>The default wait queue implementation, which uses a double-ended queue.</summary>
  /// <typeparam name="T">The type of the results. If this isn't needed, use <see cref="Object"/>.</typeparam>
  ...
  public sealed class DefaultAsyncWaitQueue<T> : IAsyncWaitQueue<T>
  {
    ...
    void IAsyncWaitQueue<T>.Dequeue(T result) => _queue.RemoveFromFront().TrySetResult(result);
