...
namespace CuteAnt.AsyncEx
{
  ...
  public sealed class AsyncCountdownEvent
  {
    ...
    /// <summary>Synchronously waits for this event to be set. This method may block the calling thread.</summary>
    /// <param name="cancellationToken">The cancellation token used to cancel the wait. If this token is already canceled, this
    /// method will first check whether the event is set.</param>
    ...
    /// <summary>Attempts to modify the current count by the specified amount. This method returns
    /// <c>false</c> if the new current count value would be invalid, or if the count has already
    /// reached zero.</summary>
    ...
    /// <summary>Attempts to add the specified value to the current count. This method returns <c>false</c> if
    /// the count is already at zero or if the new count would be greater than <see cref="Int32.MaxValue"/>.</summary>
    ...
    /// <summary>Attempts to add one to the current count. This method returns <c>false</c> if the count is
    /// already at zero or if the new count would be greater than <see cref="Int32.MaxValue"/>.</summary>
    public bool TryAddCount()
    {
      return TryAddCount(1);
    }

    /// <summary>Attempts to subtract the specified value from the current count. This method returns
    /// <c>false</c> if the count is already at zero or if the new count would be less than zero.</summary>
    /// <param name="signalCount">The amount to change the current count. This must be greater than zero.</param>
    ...
    /// <summary>Attempts to subtract one from the current count. This method returns <c>false</c> if the
    /// count is already at zero or if the new count would be less than zero.</summary>
    public bool TrySignal()
    {
      return TrySignal(1);
    }

    /// <summary>Attempts to add the specified value to the current count. This method throws <see
    /// cref="InvalidOperationException"/> if the count is already at zero or if the new count would
    /// be greater than <see cref="Int32.MaxValue"/>.</summary>
    /// <param name="signalCount">
    ...
    /// <summary>Attempts to add one to the current count. This method throws <see
    /// cref="InvalidOperationException"/> if the count is already at zero or if the new count would
    /// be greater than <see cref="Int32.MaxValue"/>.</summary>
    ...
    /// <summary>Attempts to subtract the specified value from the current count. This method throws <see
    /// cref="InvalidOperationException"/> if the count is already at zero or if the new count would
    /// be less than zero.</summary>
    /// <param name="signalCount">
    /// The amount to change the current count. This must be greater than zero.
    /// </param>
    ...
    /// <summary>Attempts to subtract one from the current count. This method throws <see
    /// cref="InvalidOperationException"/> if the count is already at zero or if the new count would
    /// be less than zero.</summary>
    public void Signal()
    {
