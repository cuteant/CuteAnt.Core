...
{
  /// <summary>Flags controlling the behavior of <see cref="AsyncLazy{T}"/>.</summary>
  [Flags]
  public enum AsyncLazyFlags
  {
    /// <summary>No special flags. The factory method is executed on a thread pool thread, and does not retry initialization on failures (failures are cached).</summary>
    None = 0x0,

    /// <summary>Execute the factory method on the calling thread.</summary>
    ExecuteOnCallingThread = 0x1,

    /// <summary>If the factory method fails, then re-run the factory method the next time instead of caching the failed task.</summary>
    RetryOnFailure = 0x2,
  }
  ...
  {
    /// <summary>The synchronization object protecting <c>_instance</c>.</summary>
    private readonly object _mutex;

    /// <summary>The factory method to call.</summary>
    ...
    /// <summary>Gets a semi-unique identifier for this asynchronous lazy instance.</summary>
    ...
    {
      return () => TaskShim.Run(factory);
    }

    /// <summary>Asynchronous infrastructure support. This method permits instances of <see cref="AsyncLazy&lt;T&gt;"/> to be await'ed.</summary>
