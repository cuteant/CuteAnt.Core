...
namespace CuteAnt.Comparers
{
	...
	public static class ComparerExtensions
	{
		...
		public static IFullComparer<T> ThenBy<T>(this IComparer<T> source, IComparer<T> thenBy, Boolean descending = false)
		{
			Contract.Ensures(Contract.Result<IFullComparer<T>>() != null);
		...
		public static IFullComparer<T> ThenBy<T, TKey>(this IComparer<T> source, Func<T, TKey> selector, IComparer<TKey> keyComparer = null, Boolean allowNulls = false, Boolean descending = false)
		{
		...
		/// <summary>Returns a comparer that uses a descending key comparer if the source comparer determines the objects are equal.</summary>
		/// <typeparam name="T">The type of objects being compared.</typeparam>
		/// <typeparam name="TKey">The type of key objects being compared.</typeparam>
		/// <param name="source">The source comparer. If this is <c>null</c>, the default comparer is used.</param>
		/// <param name="selector">The key selector. May not be <c>null</c>.</param>
		...
		/// <param name="allowNulls">A value indicating whether <c>null</c> values are passed to <paramref name="selector"/>. If <c>false</c>, then <c>null</c> values are considered less than any non-<c>null</c> values and are not passed to <paramref name="selector"/>.</param>
		/// <returns>A comparer that uses a key comparer if the source comparer determines the objects are equal.</returns>
		public static IFullComparer<T> ThenByDescending<T, TKey>(this IComparer<T> source, Func<T, TKey> selector, IComparer<TKey> keyComparer = null, Boolean allowNulls = false)
		{
			Contract.Requires(selector != null);
			Contract.Ensures(Contract.Result<IFullComparer<T>>() != null);
