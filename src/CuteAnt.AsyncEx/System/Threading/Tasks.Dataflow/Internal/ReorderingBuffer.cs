...
namespace System.Threading.Tasks.Dataflow.Internal
{
	...
	/// <remarks>This type expects the first item to be ID==0 and for all subsequent items to increase IDs sequentially.</remarks>
	...
	internal sealed class ReorderingBuffer<TOutput> : IReorderingBuffer
	{
		...
		private readonly Dictionary<Int64, KeyValuePair<Boolean, TOutput>> _reorderingBuffer = new Dictionary<Int64, KeyValuePair<Boolean, TOutput>>();
		...
		private Int64 _nextReorderedIdToOutput = 0;
		...
		private Int32 CountForDebugging { get { return _reorderingBuffer.Count; } }
		...
		internal void AddItem(Int64 id, TOutput item, Boolean itemIsValid)
		{
			...
			lock (ValueLock)
			{
				...
				else
				{
					...
					_reorderingBuffer.Add(id, new KeyValuePair<Boolean, TOutput>(itemIsValid, item));
				}
			}
		}
		...
		#region == AddItemIfNextAndTrusted ==

		/// <summary>Determines whether the specified id is next to be output, and if it is
		/// and if the item is "trusted" (meaning it may be output into the output action as-is), adds it.</summary>
		/// <param name="id">The id of the item.</param>
		/// <param name="item">The item.</param>
		/// <param name="isTrusted">Whether to allow the item to be output directly if it is the next item.</param>
		/// <returns>null if the item was added.
		/// true if the item was not added but is next in line.
		/// false if the item was not added and is not next in line.
		/// </returns>
		internal Boolean? AddItemIfNextAndTrusted(Int64 id, TOutput item, Boolean isTrusted)
		{
			Contract.Requires(id != Common.INVALID_REORDERING_ID, "This ID should never have been handed out.");
			Common.ContractAssertMonitorStatus(ValueLock, held: false);
		...
		/// <summary>Informs the reordering buffer not to expect the message with the specified id.</summary>
		/// <param name="id">The id of the message to be ignored.</param>
		public void IgnoreItem(Int64 id)
		{
		...
		private void OutputNextItem(TOutput theNextItem, Boolean itemIsValid)
		{
			...
			KeyValuePair<Boolean, TOutput> nextOutputItemWithValidity;
		...
		private sealed class DebugView
		{
			...
			public Dictionary<Int64, KeyValuePair<Boolean, TOutput>> ItemsBuffered { get { return _buffer._reorderingBuffer; } }

			/// <summary>Gets the next ID required for outputting.</summary>
			public Int64 NextIdRequired { get { return _buffer._nextReorderedIdToOutput; } }
		}
