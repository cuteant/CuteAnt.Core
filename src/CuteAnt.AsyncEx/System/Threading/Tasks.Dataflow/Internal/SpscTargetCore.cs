...
namespace System.Threading.Tasks.Dataflow.Internal
{
	...
	internal sealed class SpscTargetCore<TInput>
	{
		...
		private volatile Boolean _decliningPermanently;

		/// <summary>A task has reserved the right to run the completion routine.</summary>
		private volatile Boolean _completionReserved;

		/// <summary>The Task currently active to process the block. This field is used to synchronize between producer and consumer,
		/// and it should not be set to null once the block completes, as doing so would allow for races where the producer
		/// gets another consumer task queued even though the block has completed.</summary>
		...
		internal Int32 InputCount { get { return _messages.Count; } }
		...
		internal Boolean Post(TInput messageValue)
		{
			if (_decliningPermanently) { return false; }
		...
		private DataflowMessageStatus OfferMessage_Slow(DataflowMessageHeader messageHeader, TInput messageValue, ISourceBlock<TInput> source, Boolean consumeToAccept)
		{
		...
		#region ** ScheduleConsumerIfNecessary **
			...
			while (continueProcessing)
			{
				...
				finally
				{
					...
					else
					{
						...
						Boolean wasDecliningPermanently = _decliningPermanently;
		...
		/// <summary>Completes the target core.  If an exception is provided, the block will end up in a faulted state.
