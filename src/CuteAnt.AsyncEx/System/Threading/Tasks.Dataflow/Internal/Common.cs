...
namespace System.Threading.Tasks.Dataflow.Internal
{
	...
	internal static class Common
	{
		...
		/// <summary>An invalid ID to assign for reordering purposes.  This value is chosen to be the last of the 64-bit integers that
		/// could ever be assigned as a reordering ID.
		/// </summary>
		internal const Int64 INVALID_REORDERING_ID = -1L;
		...
		internal const Int32 SINGLE_MESSAGE_ID = 1;
		...
		/// <summary>The cached completed Task{Boolean} with a result of true.</summary>
		internal static readonly Task<Boolean> CompletedTaskWithTrueResult = TaskConstants.BooleanTrue; // CreateCachedBooleanTask(true);

		/// <summary>The cached completed Task{Boolean} with a result of false.</summary>
		internal static readonly Task<Boolean> CompletedTaskWithFalseResult = TaskConstants.BooleanFalse; // CreateCachedBooleanTask(false);
		...
		[Conditional("DEBUG")]
		internal static void ContractAssertMonitorStatus(object syncObj, Boolean held)
		{
			Debug.Assert(syncObj != null, "The monitor object to check must be provided.");
			Debug.Assert(TaskShim.IsEntered(syncObj) == held, "The locking scheme was not correctly followed.");
		}

		/// <summary>Keeping alive processing tasks: maximum number of processed messages.</summary>
		internal const Int32 KEEP_ALIVE_NUMBER_OF_MESSAGES_THRESHOLD = 1;

		/// <summary>Keeping alive processing tasks: do not attempt this many times.</summary>
		internal const Int32 KEEP_ALIVE_BAN_COUNT = 1000;
		...
		internal delegate Boolean KeepAlivePredicate<TStateIn, TStateOut>(TStateIn stateIn, out TStateOut stateOut);
		...
		internal static Boolean TryKeepAliveUntil<TStateIn, TStateOut>(
			KeepAlivePredicate<TStateIn, TStateOut> predicate, TStateIn stateIn, out TStateOut stateOut)
		{
			Debug.Assert(predicate != null, "Non-null predicate to execute is required.");
			const Int32 ITERATION_LIMIT = 16;

			for (Int32 c = ITERATION_LIMIT; c > 0; c--)
			{
				if (!Thread.Yield())
				{
		...
		internal static Int32 GetBlockId(IDataflowBlock block)
		{
			Debug.Assert(block != null, "Block required to extract an Id.");
			const Int32 NOTASKID = 0; // tasks don't have 0 as ids
			var task = Common.GetPotentiallyNotSupportedCompletionTask(block);
			return task != null ? task.Id : NOTASKID;
		}
				...
				IDataflowBlock block, DataflowBlockOptions options = null)
		{
			...
			try
			{
				return options.NameFormat.FormatWith(blockName, blockId);
			}
		...
		internal static Boolean IsCooperativeCancellation(Exception exception)
		{
		...
		[SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes")]
		internal static void StoreDataflowMessageValueIntoExceptionData<T>(Exception exc, T messageValue, Boolean targetInnerExceptions = false)
		{
		...
		internal static void AddException(ref List<Exception> list, Exception exception, Boolean unwrapInnerExceptions = false)
		{
		...
		private static Task<Boolean> CreateCachedBooleanTask(Boolean value)
		{
		...
		/// <summary>Creates an IDisposable that, when disposed, will acquire the outgoing lock while removing
		/// the target block from the target registry.</summary>
		...
		{
			return AsyncUtils.GetContinuationOptions(toInclude);
		}
																...
																ref List<Exception> exceptions)
		{
			...
			Int32 initialCount = postponedMessages.Count;
	...
	internal class BoundingState
	{
		...
		internal readonly Int32 BoundedCapacity;
		...
		internal Boolean CountIsLessThanBound { get { return CurrentCount < BoundedCapacity; } }
	}

	#endregion

	#region -- class BoundingStateWithPostponed<TInput> --
	...
	internal class BoundingStateWithPostponed<TInput> : BoundingState
	{
		...
		/// </remarks>
		internal Int32 OutstandingTransfers;

		/// <summary>Initializes the BoundingState.</summary>
		/// <param name="boundedCapacity">The positive bounded capacity.</param>
		internal BoundingStateWithPostponed(Int32 boundedCapacity)
			: base(boundedCapacity)
		{
		}

		/// <summary>Gets the number of postponed messages for the debugger.</summary>
		private Int32 PostponedMessagesCountForDebugger { get { return PostponedMessages.Count; } }
	}

	#endregion

	#region -- class BoundingStateWithPostponedAndTask<TInput> --
	...
	internal class BoundingStateWithPostponedAndTask<TInput> : BoundingStateWithPostponed<TInput>
	{
		...
		/// <summary>Initializes the BoundingState.</summary>
		/// <param name="boundedCapacity">The positive bounded capacity.</param>
		internal BoundingStateWithPostponedAndTask(Int32 boundedCapacity)
			: base(boundedCapacity)
		{
		}
	}
	...
	#region -- struct VoidResult --

	/// <summary>Type used with TaskCompletionSource(Of TResult) as the TResult
	/// to ensure that the resulting task can't be upcast to something
	/// that in the future could lead to compat problems.</summary>
