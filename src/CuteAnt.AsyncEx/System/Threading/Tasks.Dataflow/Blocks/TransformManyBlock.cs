...
namespace System.Threading.Tasks.Dataflow
{
	...
	public sealed class TransformManyBlock<TInput, TOutput> : IPropagatorBlock<TInput, TOutput>, IReceivableSourceBlock<TOutput>, IDebuggerDisplay
	{
		...
		#region ** StoreOutputItems **

		/// <summary>Stores the output items, either into the reordering buffer or into the source half.
		/// Ensures that the bounding count is correctly updated.</summary>
		/// <param name="messageWithId">The message with id.</param>
		/// <param name="outputItems">The output items to be persisted.</param>
		private void StoreOutputItems(KeyValuePair<TInput, Int64> messageWithId, IEnumerable<TOutput> outputItems)
		{
		...
		#region ** StoreOutputItemsReordered **
		...
		private void StoreOutputItemsReordered(Int64 id, IEnumerable<TOutput> item)
		{
			...
			Boolean isBounded = target.IsBounded;
			...
			var isNextNullable = _reorderingBuffer.AddItemIfNextAndTrusted(id, itemAsTrustedList, itemAsTrustedList != null);
			if (!isNextNullable.HasValue) return; // data was successfully output
			var isNextItem = isNextNullable.Value;
		...
		#region ** StoreOutputItemsNonReorderedAtomic **

		/// <summary>Stores the trusted enumerable en mass into the source core.
		/// This method does not go through the reordering buffer.</summary>
		...
		#region ** StoreOutputItemsNonReorderedWithIteration **

		/// <summary>Stores the untrusted enumerable into the source core.
		/// This method does not go through the reordering buffer.</summary>
		...
		#region ** UpdateBoundingCountWithOutputCount **

		/// <summary>Updates the bounding count based on the number of output items
		/// generated for a single input.</summary>
		/// <param name="count">The number of output items.</param>
		private void UpdateBoundingCountWithOutputCount(Int32 count)
		{
