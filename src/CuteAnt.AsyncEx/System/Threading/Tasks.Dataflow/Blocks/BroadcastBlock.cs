...
{
  /// <summary>Provides a buffer for storing at most one element at time, overwriting each message with the next as it arrives.
  /// Messages are broadcast to all linked targets, all of which may consume a clone of the message.</summary>
  ...
  public sealed class BroadcastBlock<T> : IPropagatorBlock<T, T>, IReceivableSourceBlock<T>, IDebuggerDisplay
  {
    ...
    public BroadcastBlock(Func<T, T> cloningFunction, DataflowBlockOptions dataflowBlockOptions)
    {
      ...
      Action<Int32> onItemsRemoved = null;
    ...
    internal void CompleteCore(Exception exception, Boolean storeExceptionEvenIfAlreadyCompleting, Boolean revertProcessingState = false)
    {
    ...
    private void OnItemsRemoved(Int32 numItemsRemoved)
    {
    ...
    #region == ConsumeAsyncIfNecessary ==
    ...
    internal void ConsumeAsyncIfNecessary(Boolean isReplacementReplica = false)
    {
    ...
    #region ** ConsumeMessagesLoopCore **
    ...
    private void ConsumeMessagesLoopCore()
    {
      ...
      try
      {
        ...
        for (Int32 i = 0; i < maxMessagesPerTask && ConsumeAndStoreOneMessageIfAvailable(); i++) ;
      }
    ...
    #region ** ConsumeAndStoreOneMessageIfAvailable **

    /// <summary>Retrieves one postponed message if there's room and if we can consume a postponed message.
    /// Stores any consumed message into the source half.</summary>
    ...
    #region ** CompleteTargetIfPossible **
    ...
    private Boolean HasValueForDebugger { get { return _source.GetDebuggingInformation().HasValue; } }
    ...
    private sealed class DebugView
    {
      ...
      public Boolean HasValue { get { return _broadcastBlock.HasValueForDebugger; } }
      ...
      public Boolean IsDecliningPermanently { get { return _broadcastBlock._decliningPermanently; } }
      ...
      public Int32 Id { get { return Common.GetBlockId(_broadcastBlock); } }
    ...
    #region ** class BroadcastingSourceCore<TOutput> **
    ...
    private sealed class BroadcastingSourceCore<TOutput>
    {
      ...
      private readonly Action<Int32> _itemsRemovedAction;
      ...
      private Boolean _currentMessageIsValid;
      ...
      private Boolean _enableOffering;
      ...
      private Int64 _nextMessageId = 1; // We are going to use this value before incrementing.
      ...
      internal BroadcastingSourceCore(
          ...
          Action<Int32> itemsRemovedAction)
      {
      ...
      #region * OfferCurrentMessageToNewTarget *
      ...
      private void OfferCurrentMessageToNewTarget(ITargetBlock<TOutput> target)
      {
        ...
        var useCloning = _cloningFunction != null;
        ...
        {
          Debug.Assert(result != DataflowMessageStatus.NotAvailable, "Messages from a Broadcast should never be missed.");
        }
      }
      ...
      private Boolean OfferToTargets()
      {
        ...
        var message = default(TOutput);
        var numDequeuedMessages = 0;
        lock (ValueLock)
        {
      ...
      /// <param name="isReplacementReplica">Whether this call is the continuation of a previous message loop.</param>
      private void OfferAsyncIfNecessary(Boolean isReplacementReplica = false)
      {
        Common.ContractAssertMonitorStatus(ValueLock, held: true);
        // This method must not take the OutgoingLock.

        Boolean currentlyProcessing = _taskForOutputProcessing != null;
        Boolean processingToDo = _enableOffering && _messages.Count > 0;
      ...
      private void OfferMessagesLoopCore()
      {
        try
        {
          ...
          lock (OutgoingLock)
          {
            ...
            for (Int32 counter = 0; counter < maxMessagesPerTask && !CanceledOrFaulted; counter++)
            {
      ...
      private void CompleteBlockIfPossible()
      {
        ...
        {
          var currentlyProcessing = _taskForOutputProcessing != null;
          var noMoreMessages = _decliningPermanently && _messages.Count == 0;

          // Are we done forever?
          var complete = !currentlyProcessing && (noMoreMessages || CanceledOrFaulted);
          if (complete) { CompleteBlockIfPossible_Slow(); }
        }
      }
      ...
      internal sealed class DebuggingInformation
      {
        ...
        public Boolean HasValue { get { return _source._currentMessageIsValid; } }
