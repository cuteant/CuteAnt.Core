...
namespace System.Collections.Concurrent
{
  ...
  /// <remarks>
  /// All public and protected members of <see cref="ConcurrentStackX{T}"/> are thread-safe and may be used
  /// concurrently from multiple threads.
  /// </remarks>
  [DebuggerDisplay("Count = {Count}")]
  [DebuggerTypeProxy(typeof(IProducerConsumerCollectionDebugView<>))]
  public class ConcurrentStackX<T> : IProducerConsumerCollection<T>, IReadOnlyCollection<T>
  {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ConcurrentStackX{T}"/>
    /// class.
    /// </summary>
    public ConcurrentStackX()
    {
    }
    ...
    /// cref="ConcurrentStackX{T}"/>.</param>
    /// <exception cref="T:System.ArgumentNullException">The <paramref name="collection"/> argument is
    /// null.</exception>
    public ConcurrentStackX(IEnumerable<T> collection)
    {
      if (collection == null)
      {
    ...
    /// <summary>Gets a value that indicates whether the <see cref="ConcurrentStackX{T}"/> is empty.</summary>
    /// <value>true if the <see cref="ConcurrentStackX{T}"/> is empty; otherwise, false.</value>
    /// <remarks>
    ...
    /// <summary>Gets the number of elements contained in the <see cref="ConcurrentStackX{T}"/>.</summary>
    /// <value>The number of elements contained in the <see cref="ConcurrentStackX{T}"/>.</value>
    /// <remarks>
    ...
    /// with the SyncRoot; otherwise, false. For <see cref="ConcurrentStackX{T}"/>, this property always
    /// returns false.</value>
    ...
    /// <summary>Removes all objects from the <see cref="ConcurrentStackX{T}"/>.</summary>
    public void Clear()
    {
    ...
    /// <see cref="ConcurrentStackX{T}"/>. The <see cref="T:System.Array"/> must
    ...
    /// <summary>
    /// Copies the <see cref="ConcurrentStackX{T}"/> elements to an existing one-dimensional <see
    /// cref="T:System.Array"/>, starting at the specified array index.
    /// </summary>
    /// <param name="array">The one-dimensional <see cref="T:System.Array"/> that is the destination of
    /// the elements copied from the
    /// <see cref="ConcurrentStackX{T}"/>. The <see cref="T:System.Array"/> must have zero-based
    /// indexing.</param>
    /// <param name="index">The zero-based index in <paramref name="array"/> at which copying
    /// begins.</param>
    /// <exception cref="ArgumentNullException"><paramref name="array"/> is a null reference (Nothing in
    ...
    /// -or- The number of elements in the source <see cref="ConcurrentStackX{T}"/> is greater than the
...
#pragma warning disable 0420 // No warning for Interlocked.xxx if compiled with new managed compiler (Roslyn)
    /// <summary>Inserts an object at the top of the <see cref="ConcurrentStackX{T}"/>.</summary>
    /// <param name="item">The object to push onto the <see cref="ConcurrentStackX{T}"/>. The value can be
    ...
    /// <summary>Inserts multiple objects at the top of the <see cref="ConcurrentStackX{T}"/> atomically.</summary>
    /// <param name="items">The objects to push onto the <see cref="ConcurrentStackX{T}"/>.</param>
    ...
    /// inserting elements onto the top of the <see cref="ConcurrentStackX{T}"/>.</param>
    ...
    /// <remarks>For <see cref="ConcurrentStackX{T}"/>, this operation
    /// will always insert the object onto the top of the <see cref="ConcurrentStackX{T}"/>
    ...
    /// <summary>
    /// Attempts to return an object from the top of the <see cref="ConcurrentStackX{T}"/>
    /// without removing it.
    /// </summary>
    ...
    /// <summary>Attempts to pop and return the object at the top of the <see cref="ConcurrentStackX{T}"/>.</summary>
    /// <param name="result">
    /// When this method returns, if the operation was successful, <paramref name="result"/> contains the
    /// object removed. If no object was available to be removed, the value is unspecified.
    /// </param>
    /// <returns>true if an element was removed and returned from the top of the <see
    /// cref="ConcurrentStackX{T}"/>
    ...
    /// <summary>
    /// Attempts to pop and return multiple objects from the top of the <see cref="ConcurrentStackX{T}"/>
    /// atomically.
    /// </summary>
    /// <param name="items">
    /// The <see cref="T:System.Array"/> to which objects popped from the top of the <see
    /// cref="ConcurrentStackX{T}"/> will be added.
    /// </param>
    /// <returns>The number of objects successfully popped from the top of the <see
    /// cref="ConcurrentStackX{T}"/> and inserted in
    ...
    /// </param>
    /// <param name="startIndex">The zero-based offset in <paramref name="items"/> at which to begin
    /// inserting elements from the top of the <see cref="ConcurrentStackX{T}"/>.</param>
    /// <param name="count">The number of elements to be popped from top of the <see
    /// cref="ConcurrentStackX{T}"/> and inserted into <paramref name="items"/>.</param>
    /// <returns>The number of objects successfully popped from the top of 
    /// the <see cref="ConcurrentStackX{T}"/> and inserted in <paramref name="items"/>.</returns>        
    /// <exception cref="ArgumentNullException"><paramref name="items"/> is a null reference
    /// (Nothing in Visual Basic).</exception>
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="startIndex"/> or <paramref
    /// name="count"/> is negative. Or <paramref name="startIndex"/> is greater than or equal to the length 
    ...
    /// </param>
    /// <returns>The number of objects successfully popped from the top of
    /// the <see cref="ConcurrentStackX{T}"/>.</returns>
...
#pragma warning restore 0420
    ...
    /// <summary>Local helper function to copy the poped elements into a given collection</summary>
    ...
    /// <remarks>For <see cref="ConcurrentStackX{T}"/>, this operation will attempt to pope the object at
    /// the top of the <see cref="ConcurrentStackX{T}"/>.
    /// </remarks>
    ...
    /// <summary>Copies the items stored in the <see cref="ConcurrentStackX{T}"/> to a new array.</summary>
    /// <returns>A new array containing a snapshot of elements copied from the <see
    /// cref="ConcurrentStackX{T}"/>.</returns>
    public T[] ToArray()
    {
      Node curr = _head;
      return curr == null ?
          CuteAnt.EmptyArray<T>.Instance :
          ToList(curr).ToArray();
    }
    ...
    /// <summary>Returns an enumerator that iterates through the <see cref="ConcurrentStackX{T}"/>.</summary>
    /// <returns>An enumerator for the <see cref="ConcurrentStackX{T}"/>.</returns>
    /// <remarks>
    /// The enumeration represents a moment-in-time snapshot of the contents
    /// of the stack.  It does not reflect any updates to the collection after 
    /// <see cref="IEnumerator{T}.GetEnumerator"/> was called.  The enumerator is safe to use
    ...
    /// <see cref="IEnumerable.GetEnumerator"/> was called. The enumerator is safe to use concurrently with reads
