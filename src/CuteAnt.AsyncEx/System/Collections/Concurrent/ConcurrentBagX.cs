...
{
  /// <summary>Represents an thread-safe, unordered collection of objects.</summary>
  ...
  /// duplicates. <see cref="ConcurrentBagX{T}"/> is a thread-safe bag implementation, optimized for
  /// scenarios where the same thread will be both producing and consuming data stored in the bag.
  /// </para>
  /// <para>
  /// <see cref="ConcurrentBagX{T}"/> accepts null reference (Nothing in Visual Basic) as a valid 
  /// value for reference types.
  /// </para>
  /// <para>
  /// All public and protected members of <see cref="ConcurrentBagX{T}"/> are thread-safe and may be used
  ...
  [Serializable]
  public class ConcurrentBagX<T> : IProducerConsumerCollection<T>, IReadOnlyCollection<T>
  {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ConcurrentBagX{T}"/>
    /// class.
    /// </summary>
    public ConcurrentBagX()
    {
      Initialize(null);
    }
    ...
    /// <summary>Local helper function to initialize a new bag object</summary>
    ...
    /// <summary>Adds an object to the <see cref="ConcurrentBagX{T}"/>.</summary>
    /// <param name="item">The object to be added to the
    /// <see cref="ConcurrentBagX{T}"/>. The value can be a null reference
...
#pragma warning restore 0420
    ...
    /// <summary>Attempts to add an object to the <see cref="ConcurrentBagX{T}"/>.</summary>
    /// <param name="item">The object to be added to the 
    /// <see cref="ConcurrentBagX{T}"/>. The value can be a null reference
    /// (Nothing in Visual Basic) for reference types.</param>
    ...
    /// cref="ConcurrentBagX{T}"/>.
    /// </summary>
    /// <param name="result">When this method returns, <paramref name="result"/> contains the object
    /// removed from the <see cref="ConcurrentBagX{T}"/> or the default value
    ...
    /// <summary>
    /// Attempts to return an object from the <see cref="ConcurrentBagX{T}"/>
    /// without removing it.
    /// </summary>
    /// <param name="result">When this method returns, <paramref name="result"/> contains an object from
    /// the <see cref="ConcurrentBagX{T}"/> or the default value of
    ...
    /// <summary>Local helper function to Take or Peek an item from the bag</summary>
...
#pragma warning restore 0420
    ...
    private ThreadLocalList GetThreadList(bool forceCreate)
    {
      ...
      else if (forceCreate)
      {
        ...
        lock (GlobalListsLock)
        {
          if (_headList == null)
          {
            list = new ThreadLocalList(TaskShim.CurrentManagedThreadId);
            _headList = list;
            _tailList = list;
          }
          else
    ...
    private ThreadLocalList GetUnownedList()
    {
      ...
      Debug.Assert(TaskShim.IsEntered(GlobalListsLock));

      int currentThreadId = TaskShim.CurrentManagedThreadId;
    ...
    /// <summary>local helper function tries to steal an item from given local list</summary>
    ...
    /// cref="ConcurrentBagX{T}"/> is greater than the available space from
    ...
    /// <summary>Copies the <see cref="ConcurrentBagX{T}"/> elements to a new array.</summary>
    /// <returns>A new array containing a snapshot of elements copied from the <see
    /// cref="ConcurrentBagX{T}"/>.</returns>
    public T[] ToArray()
    {
      // Short path if the bag is empty
      if (_headList == null)
        return CuteAnt.EmptyArray<T>.Instance;
    ...
    {
      return ((ConcurrentBagX<T>)this).GetEnumerator();
    }

    /// <summary>Gets the number of elements contained in the <see cref="ConcurrentBagX{T}"/>.</summary>
    /// <value>The number of elements contained in the <see cref="ConcurrentBagX{T}"/>.</value>
    /// <remarks>
    ...
    /// <summary>Gets a value that indicates whether the <see cref="ConcurrentBagX{T}"/> is empty.</summary>
    /// <value>true if the <see cref="ConcurrentBagX{T}"/> is empty; otherwise, false.</value>
    public bool IsEmpty
    {
      get
      {
    ...
    /// with the SyncRoot; otherwise, false. For <see cref="ConcurrentBagX{T}"/>, this property always
    /// returns false.</value>
    ...
    /// <summary>Local helper method to unfreeze the bag from a frozen state</summary>
    ...
    /// <summary>local helper method to acquire all local lists locks</summary>
    ...
    /// <summary>Local helper method to release all local lists locks</summary>
    ...
    /// <summary>Local helper function to wait all unsynchronized operations</summary>
    ...
    /// <summary>Local helper function to get the bag count, the caller should call it from Freeze/Unfreeze block</summary>
    ...
    /// <summary>A class that represents a node in the lock thread list</summary>
    internal class Node
    {
      public Node(T value)
      {
    ...
    internal class ThreadLocalList
    {
      ...
      /// <summary>ThreadLocalList constructor</summary>
      /// <param name="ownerThreadId">The owner thread for this list</param>
      internal ThreadLocalList(int ownerThreadId)
      {
        _ownerThreadId = ownerThreadId;
      }
      /// <summary>Add new item to head of the list</summary>
      ...
      /// <summary>Remove an item from the head of the list</summary>
      ...
      /// <summary>Peek an item from the head of the list</summary>
      ...
      /// <summary>Steal an item from the tail of the list</summary>
      /// <param name="result">the removed item</param>
      ...
      /// <summary>Gets the total list count, it's not thread safe, may provide incorrect count if it is called concurrently</summary>
      internal int Count
      {
        get
        {
  ...
  /// <summary>List operations for ConcurrentBag</summary>
