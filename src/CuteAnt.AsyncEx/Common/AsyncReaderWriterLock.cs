...
namespace CuteAnt.AsyncEx
{
	...
	public sealed class AsyncReaderWriterLock
	{
		...
		/// <summary>The queue of TCSs that other tasks are awaiting to acquire the lock as upgradeable readers.</summary>
		private readonly IAsyncWaitQueue<UpgradeableReaderKey> _upgradeableReaderQueue;

		/// <summary>The queue of TCSs that other tasks are awaiting to upgrade a reader lock to a writer lock.</summary>
		...
		/// <summary>The current upgradeable reader lock key, if any. If this is not <c>null</c>, 
		/// then there is an upgradeable reader lock held.</summary>
		private UpgradeableReaderKey _upgradeableReaderKey;

		/// <summary>Number of reader locks held (including an upgradeable reader lock, 
		/// if applicable); -1 if a writer lock is held; 0 if no locks are held.</summary>
		private Int32 _locksHeld;
		...
		[DebuggerNonUserCode]
		internal Int32 GetReaderCountForDebugger { get { return (_locksHeld > 0 ? _locksHeld : 0); } }

		[DebuggerNonUserCode]
		internal Boolean GetUpgradeInProgressForDebugger { get { return !_upgradeReaderQueue.IsEmpty; } }
		...
		/// <summary>Applies a continuation to the task that will call <see cref="ReleaseWaiters"/> 
		/// if the task is canceled. This method may not be called while holding the sync lock.</summary>
		...
		/// <summary>Asynchronously acquires the lock as a reader. 
		/// Returns a disposable that releases the lock when disposed.</summary>
		/// <param name="cancellationToken">The cancellation token used to cancel the lock. 
		/// If this is already set, then this method will attempt to take the lock immediately (succeeding if the lock is currently available).</param>
		...
		public AwaitableDisposable<IDisposable> ReaderLockAsync(CancellationToken cancellationToken)
		{
			...
			lock (SyncObject)
			{
				// If the lock is available or in read mode and there are no waiting writers, 
				// upgradeable readers, or upgrading readers, take it immediately.
		...
		/// <summary>Synchronously acquires the lock as a reader. 
		...
		/// <summary>Asynchronously acquires the lock as a writer. 
		/// Returns a disposable that releases the lock when disposed.</summary>
		/// <param name="cancellationToken">The cancellation token used to cancel the lock. If this is already set, 
		/// then this method will attempt to take the lock immediately (succeeding if the lock is currently available).</param>
		/// <returns>A disposable that releases the lock when disposed.</returns>
		...
		/// <summary>Synchronously acquires the lock as a writer. Returns a disposable that releases the lock when disposed. 
		/// This method may block the calling thread.</summary>
		/// <param name="cancellationToken">The cancellation token used to cancel the lock. If this is already set, 
		/// then this method will attempt to take the lock immediately (succeeding if the lock is currently available).</param>
		/// <returns>A disposable that releases the lock when disposed.</returns>
		...
		/// <summary>Asynchronously acquires the lock as a writer. Returns a disposable that releases the lock when disposed. 
		/// This method may block the calling thread.</summary>
		/// <returns>A disposable that releases the lock when disposed.</returns>
		...
		/// <summary>Asynchronously acquires the lock as a reader with the option to upgrade. 
		/// Returns a key that can be used to upgrade and downgrade the lock, and releases the lock when disposed.</summary>
		/// <param name="cancellationToken">The cancellation token used to cancel the lock. 
		/// If this is already set, then this method will attempt to take the lock immediately (succeeding if the lock is currently available).</param>
		...
		/// <summary>Synchronously acquires the lock as a reader with the option to upgrade. 
		...
		/// <summary>Asynchronously upgrades a reader lock to a writer lock. This method assumes the sync lock is already held.</summary>
		...
		/// <summary>Downgrades a writer lock to a reader lock. This method assumes the sync lock is already held.</summary>
		...
		/// <summary>Releases the lock as an upgradeable reader.</summary>
		...
		/// <summary>The disposable which manages the upgradeable reader lock.</summary>
		...
		public sealed class UpgradeableReaderKey : IDisposable
		{
			...
			/// <summary>The task doing the upgrade.</summary>
			private Task<IDisposable> _upgrade;

			/// <summary>A task that is completed with the upgrade key object for this key.</summary>
			...
			/// <summary>Gets a value indicating whether this lock has been upgraded to a write lock.</summary>
			...
			/// <summary>Upgrades the reader lock to a writer lock. 
			/// Returns a disposable that downgrades the writer lock to a reader lock when disposed.</summary>
			/// <param name="cancellationToken">The cancellation token used to cancel the upgrade. 
			/// If this is already set, then this method will attempt to upgrade immediately (succeeding if the lock is currently available).</param>
			...
			/// <summary>Synchronously upgrades the reader lock to a writer lock. 
			...
			/// <summary>Downgrades the writer lock to a reader lock.</summary>
			...
			/// <summary>The disposable which downgrades an upgradeable reader key.</summary>
			internal sealed class UpgradeKey : IDisposable
			{
				/// <summary>The upgradeable reader key to downgrade.</summary>
				private readonly UpgradeableReaderKey _key;

				/// <summary>Creates the upgrade key for an upgradeable reader key.</summary>
				/// <param name="key">The upgradeable reader key to downgrade. May not be <c>null</c>.</param>
				internal UpgradeKey(UpgradeableReaderKey key)
				{
					_key = key;
				}

				/// <summary>Downgrade the upgradeable reader key.</summary>
				public void Dispose()
				{
					_key.Downgrade();
				}
		...
		private sealed class DebugView
		{
			...
			public Int32 Id { get { return _rwl.Id; } }

			public State State { get { return _rwl.GetStateForDebugger; } }

			public Int32 ReaderCount { get { return _rwl.GetReaderCountForDebugger; } }

			public Boolean UpgradeInProgress { get { return _rwl.GetUpgradeInProgressForDebugger; } }
