...
{
	/// <summary>The source (controller) of a "pause token", which can be used to cooperatively pause and unpause operations.</summary>
	public sealed class PauseTokenSource
	{
		/// <summary>The MRE that manages the "pause" logic. When the MRE is set, the token is not paused; 
		/// when the MRE is not set, the token is paused.</summary>
		private readonly AsyncManualResetEvent _mre = new AsyncManualResetEvent(true);

		/// <summary>Whether or not this source (and its tokens) are in the paused state.</summary>
		...
		/// <summary>Gets a pause token controlled by this source.</summary>
	...
	{
		/// <summary>The MRE that manages the "pause" logic, or <c>null</c> if this token can never be paused. 
		/// When the MRE is set, the token is not paused; when the MRE is not set, the token is paused.</summary>
		...
		/// <summary>Whether this token can ever possibly be paused.</summary>
		public Boolean CanBePaused
		{
			get { return _mre != null; }
		}

		/// <summary>Whether or not this token is in the paused state.</summary>
		public Boolean IsPaused
		{
		...
		/// <param name="token">The cancellation token to observe. If the token is already canceled, 
		/// this method will first check if the pause token is unpaused, and will return without an exception in that case.</param>
		...
		/// <summary>Synchronously waits until the pause token is not paused, or until this wait is canceled by the cancellation token.</summary>
		/// <param name="token">The cancellation token to observe. If the token is already canceled, 
		/// this method will first check if the pause token is unpaused, and will return without an exception in that case.</param>
