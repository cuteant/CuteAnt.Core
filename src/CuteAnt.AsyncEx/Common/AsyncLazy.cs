...
namespace CuteAnt.AsyncEx
{
	...
	public sealed class AsyncLazy<T>
	{
		...
		/// <param name="factory">The delegate that is invoked on a background thread to produce the value when it is needed. 
		...
		/// <summary>Initializes a new instance of the <see cref="AsyncLazy&lt;T&gt;"/> class.</summary>
		/// <param name="factory">The asynchronous delegate that is invoked on a background thread to produce the value when it is needed. 
		/// May not be <c>null</c>.</param>
		...
		/// <summary>Whether the asynchronous factory method has started. This is initially <c>false</c> and becomes <c>true</c> 
		/// when this instance is awaited or after <see cref="Start"/> is called.</summary>
		...
		internal sealed class DebugView
		{
			...
			internal DebugView(AsyncLazy<T> lazy)
			{
				_lazy = lazy;
			}

			internal LazyState State { get { return _lazy.GetStateForDebugger; } }
