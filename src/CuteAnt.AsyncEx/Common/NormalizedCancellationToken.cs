...
{
  /// <summary>A cancellation token wrapper that may represent multiple linked cancellation tokens. 
  /// Instances of this type should always be disposed.</summary>
  public sealed class NormalizedCancellationToken : SingleDisposable<object>
  {
    /// <summary>The <see cref="CancellationTokenSource"/>, if any. If this is not <c>null</c>, 
    /// then <see cref="_token"/> is <c>_cts.Token</c>.</summary>
    private readonly CancellationTokenSource _cts;

    /// <summary>The <see cref="Token"/>. If <see cref="_cts"/> is not <c>null</c>, then this is <c>_cts.Token</c>.</summary>
    private readonly CancellationToken _token;

    /// <summary>Creates a normalized cancellation token that can never be canceled.</summary>
    public NormalizedCancellationToken()
      : this(null)
    {
    }

    /// <summary>Creates a normalized cancellation token from a <see cref="CancellationTokenSource"/>. 
    /// <see cref="Token"/> is set to the <see cref="CancellationTokenSource.Token"/> property of <paramref name="cts"/>.</summary>
    /// <param name="cts">The source for this token. May be <c>null</c> to create a normalized cancellation token 
    /// that can never be canceled.</param>
    ...
    /// <summary>Creates a normalized cancellation token from a <see cref="CancellationToken"/>. 
    /// <see cref="Token"/> is set to <paramref name="token"/>.</summary>
    ...
    /// <summary>Releases any resources used by this normalized cancellation token.</summary>
    ...
    /// <summary>Gets the <see cref="CancellationToken"/> for this normalized cancellation token.</summary>
