...
{
	/// <summary>A collection of cancelable <see cref="TaskCompletionSource{T}"/> instances. 
	/// Implementations must be threadsafe <b>and</b> must work correctly if the caller is holding a lock.</summary>
	...
	public interface IAsyncWaitQueue<T>
	{
		...
		/// <summary>Creates a new entry and queues it to this wait queue. 
		/// The returned task must support both synchronous and asynchronous waits.</summary>
		/// <returns>The queued task.</returns>
		Task<T> Enqueue();

		/// <summary>Removes a single entry in the wait queue. Returns a disposable that completes that entry.</summary>
		...
		/// <summary>Removes all entries in the wait queue. Returns a disposable that completes all entries.</summary>
		...
		/// <summary>Removes all entries from the wait queue. Returns a disposable that cancels all entries.</summary>
	...
	{
		/// <summary>Creates a new entry and queues it to this wait queue. If the cancellation token is already canceled, 
		/// this method immediately returns a canceled task without modifying the wait queue.</summary>
		...
		{
			if (token.IsCancellationRequested) { return TaskConstants<T>.Canceled; }
	...
	public sealed class DefaultAsyncWaitQueue<T> : IAsyncWaitQueue<T>
	{
		...
		Boolean IAsyncWaitQueue<T>.IsEmpty
		{
			get { return Count == 0; }
		}
		...
		internal sealed class DebugView
		{
			...
			internal DebugView(DefaultAsyncWaitQueue<T> queue)
			{
				_queue = queue;
			}

			[DebuggerBrowsable(DebuggerBrowsableState.RootHidden)]
			internal Task<T>[] Tasks
			{
