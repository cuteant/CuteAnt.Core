...
namespace CuteAnt.AsyncEx
{
	...
	/// <remarks><see cref="Execute()"/> may only be called once. After <see cref="Execute()"/> returns, 
	/// the async context should be disposed.</remarks>
	...
	public sealed partial class AsyncContext : IDisposable
	{
		...
		/// <summary>The queue holding the actions to run.</summary>
		private readonly TaskQueue _queue;

		/// <summary>The <see cref="SynchronizationContext"/> for this <see cref="AsyncContext"/>.</summary>
		private readonly AsyncContextSynchronizationContext _synchronizationContext;

		/// <summary>The <see cref="TaskScheduler"/> for this <see cref="AsyncContext"/>.</summary>
		private readonly AsyncContextTaskScheduler _taskScheduler;

		/// <summary>The <see cref="TaskFactory"/> for this <see cref="AsyncContext"/>.</summary>
		private readonly TaskFactory _taskFactory;

		/// <summary>The number of outstanding operations, including actions in the queue.</summary>
		private Int32 _outstandingOperations;
		...
		/// <summary>Gets the current <see cref="AsyncContext"/> for this thread, or <c>null</c> 
		/// if this thread is not currently running in an <see cref="AsyncContext"/>.</summary>
		public static AsyncContext Current
		{
			get
			{
				var syncContext = SynchronizationContext.Current as AsyncContextSynchronizationContext;
				if (syncContext == null) { return null; }
		...
		/// From inside <see cref="Execute"/>, this value is always equal to <see cref="System.Threading.SynchronizationContext.Current"/>.</summary>
		[System.ComponentModel.EditorBrowsable(System.ComponentModel.EditorBrowsableState.Never)]
		...
		/// Be careful with async delegates; you may need to call <see cref="M:System.Threading.SynchronizationContext.OperationStarted"/> and <see cref="M:System.Threading.SynchronizationContext.OperationCompleted"/> to prevent early termination of this <see cref="AsyncContext"/>.</summary>
		[System.ComponentModel.EditorBrowsable(System.ComponentModel.EditorBrowsableState.Never)]
		...
		/// <summary>Queues a task for execution, and begins executing all tasks in the queue. 
		/// This method returns when all tasks have been completed and the outstanding asynchronous operation count is zero. 
		/// This method will unwrap and propagate errors from the task.</summary>
		...
		/// <summary>Queues a task for execution, and begins executing all tasks in the queue. 
		/// This method returns when all tasks have been completed and the outstanding asynchronous operation count is zero. 
		/// Returns the result of the task. This method will unwrap and propagate errors from the task.</summary>
		...
		/// <summary>Queues a task for execution, and begins executing all tasks in the queue. 
		/// This method returns when all tasks have been completed and the outstanding asynchronous operation count is zero. 
		/// This method will unwrap and propagate errors from the task proxy.</summary>
		/// <param name="action">The action to execute. May not be <c>null</c>.</param>
		...
		/// <summary>Queues a task for execution, and begins executing all tasks in the queue. 
		/// This method returns when all tasks have been completed and the outstanding asynchronous operation count is zero. 
		/// Returns the result of the task proxy. This method will unwrap and propagate errors from the task proxy.</summary>
		/// <typeparam name="TResult">The result type of the task.</typeparam>
		/// <param name="action">The action to execute. May not be <c>null</c>.</param>
		...
		/// <summary>Decrements the outstanding asynchronous operation count.</summary>
		...
		/// If all tasks have been completed and the outstanding asynchronous operation count is zero, then this method has undefined behavior.</summary>
		...
		private void Enqueue(Task task, Boolean propagateExceptions)
		{
		...
		/// <summary>Executes all queued actions. This method returns when all tasks have been completed 
		/// and the outstanding asynchronous operation count is zero. This method will unwrap and propagate 
		/// errors from tasks that are supposed to propagate errors.</summary>
		[System.ComponentModel.EditorBrowsable(System.ComponentModel.EditorBrowsableState.Never)]
		public void Execute()
		{
		...
		internal sealed class DebugView
		{
			...
			internal DebugView(AsyncContext context)
			{
				_context = context;
			}
