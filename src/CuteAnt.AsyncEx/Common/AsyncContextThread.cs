...
namespace CuteAnt.AsyncEx
{
	...
	public sealed class AsyncContextThread : IDisposable
	{
		...
		/// <summary>A flag used to ensure we only call <see cref="AsyncContext.OperationCompleted"/> 
		/// once during complex join/dispose operations.</summary>
		private Int32 _stoppingFlag;

		/// <summary>Initializes a new instance of the <see cref="AsyncContextThread"/> class, 
		/// creating a child thread waiting for commands.</summary>
		...
		/// <summary>Initializes a new instance of the <see cref="AsyncContextThread"/> class, 
		/// creating a child thread waiting for commands. If <paramref name="sta"/> is <c>true</c>, 
		/// then the child thread is an STA thread (throwing <see cref="NotSupportedException"/> if the platform does not support STA threads).</summary>
		public AsyncContextThread(Boolean sta)
		{
		...
		/// <summary>Requests the thread to exit and returns a task representing the exit of the thread. 
		/// The thread will exit when all outstanding asynchronous operations complete.</summary>
		...
		internal sealed class DebugView
		{
			...
			internal DebugView(AsyncContextThread thread)
			{
				_thread = thread;
			}

			internal AsyncContext Context
			{
