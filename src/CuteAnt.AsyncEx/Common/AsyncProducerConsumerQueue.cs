...
{
	/// <summary>An async-compatible producer/consumer queue.</summary>
	...
	{
		/// <summary>The underlying queue.</summary>
		private readonly Queue<T> _queue;

		/// <summary>The maximum number of elements allowed in the queue.</summary>
		private readonly Int32 _maxCount;

		/// <summary>The mutual-exclusion lock protecting the queue.</summary>
		private readonly AsyncLock _mutex;

		/// <summary>A condition variable that is signalled when the queue is not full.</summary>
		private readonly AsyncConditionVariable _notFull;

		/// <summary>A condition variable that is signalled when the queue is completed or not empty.</summary>
		private readonly AsyncConditionVariable _completedOrNotEmpty;

		/// <summary>A cancellation token source that is canceled when the queue is marked completed for adding.</summary>
		private readonly CancellationTokenSource _completed;

		/// <summary>A cached result that is common when calling <see cref="o:AsyncProducerConsumerQueueExtensions.TryDequeueFromAnyAsync"/>.</summary>
		...
		public AsyncProducerConsumerQueue(IEnumerable<T> collection, Int32 maxCount)
		{
			if (maxCount <= 0)
		...
		/// <summary>Creates a new async-compatible producer/consumer queue with a maximum element count.</summary>
		/// <param name="maxCount">The maximum element count. This must be greater than zero.</param>
		public AsyncProducerConsumerQueue(Int32 maxCount)
			: this(null, maxCount)
		{
		}

		/// <summary>Creates a new async-compatible producer/consumer queue.</summary>
		...
		private Boolean Empty { get { return _queue.Count == 0; } }

		/// <summary>Whether the queue is full.</summary>
		private Boolean Full { get { return _queue.Count == _maxCount; } }

		/// <summary>Releases resources held by this instance. After disposal, any use of this instance is undefined.</summary>
		public void Dispose()
		{
			_completed.Dispose();
		}

		/// <summary>Asynchronously marks the producer/consumer queue as complete for adding.</summary>
		...
		/// <summary>Synchronously marks the producer/consumer queue as complete for adding.</summary>
		...
		/// <summary>Attempts to enqueue an item.</summary>
		/// <param name="item">The item to enqueue.</param>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the enqueue operation. 
		/// If <paramref name="abort"/> is not <c>null</c>, then this token must include signals from the <paramref name="abort"/> object.</param>
		/// <param name="abort">A synchronization object used to cancel related enqueue operations.
		/// May be <c>null</c> if this is the only enqueue operation.</param>
		internal async Task<AsyncProducerConsumerQueue<T>> TryEnqueueAsync(T item, CancellationToken cancellationToken, TaskCompletionSource abort)
		{
			try
			{
				...
				using (await _mutex.LockAsync().ConfigureAwait(false))
				{
					...
					// Explicitly check whether the queue has been marked complete to prevent a race condition 
					// where notFull is signalled at the same time the queue is marked complete.
					if (_completed.IsCancellationRequested)
						return null;
		...
		/// <summary>Attempts to enqueue an item. This method may block the calling thread.</summary>
		/// <param name="item">The item to enqueue.</param>
		...
		/// <summary>Attempts to enqueue an item to the producer/consumer queue. 
		/// Returns <c>false</c> if the producer/consumer queue has completed adding.</summary>
		/// <param name="item">The item to enqueue.</param>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the enqueue operation.</param>
		public async Task<Boolean> TryEnqueueAsync(T item, CancellationToken cancellationToken)
		{
		...
		/// This method may block the calling thread.</summary>
		/// <param name="item">The item to enqueue.</param>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the enqueue operation.</param>
		public Boolean TryEnqueue(T item, CancellationToken cancellationToken)
		{
		...
		/// Returns <c>false</c> if the producer/consumer queue has completed adding.</summary>
		/// <param name="item">The item to enqueue.</param>
		public Task<Boolean> TryEnqueueAsync(T item)
		{
		...
		/// <summary>Enqueues an item to the producer/consumer queue. 
		/// Throws <see cref="InvalidOperationException"/> if the producer/consumer queue has completed adding.</summary>
		/// <param name="item">The item to enqueue.</param>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the enqueue operation.</param>
		...
		/// Returns <c>false</c> if the producer/consumer queue has completed adding and there are no more items.</summary>
		...
		/// <param name="cancellationToken">A cancellation token that can be used to abort the dequeue operation. 
		/// If <paramref name="abort"/> is not <c>null</c>, then this token must include signals from the <paramref name="abort"/> object.</param>
		/// <param name="abort">A synchronization object used to cancel related dequeue operations. 
		/// May be <c>null</c> if this is the only dequeue operation.</param>
		...
		/// <summary>Attempts to dequeue an item. This method may block the calling thread.</summary>
		...
		/// <summary>Attempts to dequeue an item from the producer/consumer queue.</summary>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the dequeue operation.</param>
		...
		/// <summary>Attempts to dequeue an item from the producer/consumer queue. This method may block the calling thread.</summary>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the dequeue operation.</param>
		...
		/// <summary>Dequeues an item from the producer/consumer queue. Returns the dequeued item. 
		/// Throws <see cref="InvalidOperationException"/> if the producer/consumer queue has completed adding and is empty.</summary>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the dequeue operation.</param>
		...
		/// <summary>Dequeues an item from the producer/consumer queue. Returns the dequeued item. 
		/// This method may block the calling thread. Throws <see cref="InvalidOperationException"/> 
		/// if the producer/consumer queue has completed adding and is empty.</summary>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the dequeue operation.</param>
		...
		/// <summary>The result of a <c>TryDequeue</c>, <c>DequeueFromAny</c>, or <c>TryDequeueFromAny</c> operation.</summary>
		public sealed class DequeueResult
		{
			...
			/// <summary>The queue from which the item was dequeued, or <c>null</c> if the operation failed.</summary>
			public AsyncProducerConsumerQueue<T> Queue { get; private set; }

			/// <summary>Whether the operation was successful. This is <c>true</c> if and only if <see cref="Queue"/> is not <c>null</c>.</summary>
			public Boolean Success { get { return Queue != null; } }

			/// <summary>The dequeued item. This is only valid if <see cref="Queue"/> is not <c>null</c>.</summary>
		...
		internal sealed class DebugView
		{
			...
			internal DebugView(AsyncProducerConsumerQueue<T> queue)
			{
				_queue = queue;
			}
	...
	/// <summary>Provides methods for working on multiple <see cref="AsyncProducerConsumerQueue{T}"/> instances.</summary>
	public static class AsyncProducerConsumerQueueExtensions
	{
		/// <summary>Attempts to enqueue an item to any of a number of producer/consumer queues. 
		/// Returns the producer/consumer queue that received the item. 
		/// Returns <c>null</c> if all producer/consumer queues have completed adding.</summary>
		...
		/// <summary>Enqueues an item to any of a number of producer/consumer queues. 
		/// Returns the producer/consumer queue that received the item. 
		/// Throws <see cref="InvalidOperationException"/> if all producer/consumer queues have completed adding.</summary>
		/// <param name="queues">The producer/consumer queues.</param>
		/// <param name="item">The item to enqueue.</param>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the enqueue operation.</param>
		/// <returns>The producer/consumer queue that received the item.</returns>
		...
		/// <summary>Attempts to dequeue an item from any of a number of producer/consumer queues. 
		/// The operation "fails" if all the producer/consumer queues have completed adding and are empty.</summary>
		/// <param name="queues">The producer/consumer queues.</param>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the dequeue operation.</param>
		...
		/// <summary>Dequeues an item from any of a number of producer/consumer queues. 
		/// Throws <see cref="InvalidOperationException"/> if all the producer/consumer queues have completed adding and are empty.</summary>
		/// <param name="queues">The producer/consumer queues.</param>
		/// <param name="cancellationToken">A cancellation token that can be used to abort the dequeue operation.</param>
