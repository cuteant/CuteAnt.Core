...
using CuteAnt.Serialization.Core.Xml;
...
namespace CuteAnt.Serialization.Advanced
{
	...
	public sealed class XmlPropertyDeserializer : IPropertyDeserializer
	{
		...
		private readonly Dictionary<Int32, ReferenceTargetProperty> _propertyCache =
				new Dictionary<Int32, ReferenceTargetProperty>();
		...
		private Property deserialize(PropertyArt propertyArt, Type expectedType)
		{
			...
			Int32 referenceId = _reader.GetAttributeAsInt(Attributes.ReferenceId);
		...
		{
			// ElementType
			property.ElementType = property.Type != null ? CuteAnt.Serialization.Serializing.TypeInfo.GetTypeInfo(property.Type).ElementType : null;
		...
		private void parseDictionaryProperty(DictionaryProperty property)
		{
			...
			{
				var typeInfo = CuteAnt.Serialization.Serializing.TypeInfo.GetTypeInfo(property.Type);
		...
		{
			Int32[] indexes = _reader.GetAttributeAsArrayOfInt(Attributes.Indexes);
		...
		private Property createProperty(Int32 referenceId, String propertyName, Type propertyType)
		{
