...
namespace CuteAnt.Serialization.Advanced
{
	...
	public sealed class DefaultXmlReader : IXmlReader
	{
														...
														XmlReaderSettings settings)
		{
			...
			ValidationHelper.ArgumentNull(valueConverter, "valueConverter");
		...
		public Int32[] GetAttributeAsArrayOfInt(String attributeName)
		{
			if (!_currentReader.MoveToAttribute(attributeName)) { return null; }
		...
		{
			_readerStack = new Stack<XmlReader>();
			XmlReader reader = XmlReader.Create(inputUri, _settings);
		...
		{
			_readerStack = new Stack<XmlReader>();
			XmlReader reader = XmlReader.Create(input, _settings);
