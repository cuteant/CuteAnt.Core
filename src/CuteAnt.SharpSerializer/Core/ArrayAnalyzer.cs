...
namespace CuteAnt.Serialization.Core
{
	...
	public class ArrayAnalyzer
	{
		...
		/// <returns></returns>
		private Int32 getRank(Type arrayType)
		{
		...
		private Int32 getLength(Int32 dimension, Type arrayType)
		{
			MethodInfo methodInfo = arrayType.GetMethod("GetLength");
			var length = (Int32)methodInfo.Invoke(_array, new object[] { dimension });
			return length;
		}
		...
		/// <returns></returns>
		/// <param name="arrayType"></param>
		private Int32 getLowerBound(Int32 dimension, Type arrayType)
		{
			return getBound("GetLowerBound", dimension, arrayType);
		}

		//        private Int32 getUpperBound(Int32 dimension)
		//        {
		// Not used, as UpperBound is equal LowerBound+Length
		//            return getBound("GetUpperBound", dimension);
		//        }
		private Int32 getBound(String methodName, Int32 dimension, Type arrayType)
		{
			MethodInfo methodInfo = arrayType.GetMethod(methodName);
			var bound = (Int32)methodInfo.Invoke(_array, new object[] { dimension });
			return bound;
		}
		...
		private ArrayInfo getArrayInfo(Type arrayType)
		{
			...
			for (Int32 dimension = 0; dimension < getRank(arrayType); dimension++)
			{
		...
		///</summary>
		///<returns></returns>
		public IEnumerable<Int32[]> GetIndexes()
		{
			if (_indexes == null)
			{
				_indexes = new List<Int32[]>();
		...
		private void addIndexes(Int32[] obj)
		{
			_indexes.Add(obj);
		}
		...
		public void ForEach(Action<Int32[]> action)
		{
			DimensionInfo dimensionInfo = _arrayInfo.DimensionInfos[0];

			for (Int32 index = dimensionInfo.LowerBound; index < dimensionInfo.LowerBound + dimensionInfo.Length; index++)
			{
		...
		/// <param name = "action"></param>
		private void forEach(IList<DimensionInfo> dimensionInfos, Int32 dimension, IEnumerable<Int32> coordinates,
												 Action<Int32[]> action)
		{
			...
			for (Int32 index = dimensionInfo.LowerBound; index < dimensionInfo.LowerBound + dimensionInfo.Length; index++)
			{
				var result = new List<Int32>(coordinates);
