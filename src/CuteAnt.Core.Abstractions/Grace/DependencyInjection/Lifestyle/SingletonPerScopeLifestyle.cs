...
{
    /// <summary>Singleton per scope</summary>
    ...
    public class SingletonPerScopeLifestyle : ICompiledLifestyle
    {
        ...
        /// <summary>Unique int id value</summary>
        ...
        /// <param name="threadSafe">Under normal circumstances set to false, only set to true if you know there are multiple
        /// threads accessing scope and you need to make sure only one instance is created</param>
        public SingletonPerScopeLifestyle(bool threadSafe) => ThreadSafe = threadSafe;
        ...
        /// <returns></returns>
        public virtual ICompiledLifestyle Clone() => new SingletonPerScopeLifestyle(ThreadSafe);
        ...
        public virtual IActivationExpressionResult ProvideLifestyleExpression(IInjectionScope scope, IActivationExpressionRequest request, Func<IActivationExpressionRequest, IActivationExpressionResult> activationExpression)
        {
            ...
            var local = request.PerDelegateData.GetExtraDataOrDefaultValue<ParameterExpression>(_localKey + UniqueId);
            ...
            const string _getValueFromScopeThreadSafeMethodName = nameof(SingletonPerScopeLifestyle.GetValueFromScopeThreadSafe);
            const string _getOrCreateScopedServiceMethodName = nameof(IExportLocatorScope.GetOrCreateScopedService);
            Expression createExpression;

            if (ThreadSafe)
            {
                var getValueFromScopeMethod = typeof(SingletonPerScopeLifestyle).GetRuntimeMethod(_getValueFromScopeThreadSafeMethodName, new[]
                    {
            ...
            else
            {
                ...
                if (invokeMethodFastLane)
                {
                    var getOrCreateMethod = typeof(IExportLocatorScope).GetTypeInfo()
                        .GetDeclaredMethods(_getOrCreateScopedServiceMethodName).First(m => m.GetParameters().Length == 2);
                ...
                {
                    var getOrCreateMethod = typeof(IExportLocatorScope).GetRuntimeMethod(_getOrCreateScopedServiceMethodName,
