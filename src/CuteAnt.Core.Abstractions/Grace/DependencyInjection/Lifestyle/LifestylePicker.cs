...
{
    /// <summary>Class used to configure lifestyles</summary>
    ...
    public class LifestylePicker<T> : ILifestylePicker<T>
    {
        ...
        /// <summary>Use custom lifestyle</summary>
        /// <param name="lifestyle"></param>
        /// <returns></returns>
        public T Custom(ICompiledLifestyle lifestyle)
        {
            if (lifestyle != null) { _addLifestyle(lifestyle); }

            return _returnValue;
        }

        /// <summary>Create one instance for the container including all child scopes and lifetime scopes</summary>
        ...
        /// <summary>Create one instance per object graph</summary>
        /// <param name="locking">by default no lock is used and it's assumed there is only one thread of execution in the
        /// object graph, set to true only if you believe there are multiple threads accessing one object graph</param>
        /// <returns></returns>
        ...
        /// <summary>Create one instance per request, usually a lock is not needed</summary>
        /// <param name="locking">by default no lock is used and it's assumed there is only one thread of execution in the
        /// request, set to true only if you believe there are multple threads accesing this export</param>
        /// <returns></returns>
        ...
        /// <summary>Create one instance per scope, by default no lock is used if you think it's possible multiple
        /// threads will resolve form a scope then set locking to true</summary>
        ...
        /// <summary>Create one instance per key</summary>
        ...
        /// <summary>Create one instance per named scope,</summary>
        ...
        /// <summary>Create one instance per container but don't hold a strong reference</summary>
        /// <returns></returns>
        ...
        /// <summary>Create one instance per ancestor in the object graph</summary>
        ...
        /// object graph, set to true only if you believe there are multiple threads accessing one object graph</param>
        /// <returns></returns>
        public T SingletonPerAncestor<TAncestor>(bool locking = false) => SingletonPerAncestor(typeof(TAncestor), locking);
    }
}