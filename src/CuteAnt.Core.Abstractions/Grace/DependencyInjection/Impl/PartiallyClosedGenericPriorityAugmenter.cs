...
{
  /// <summary>Increases the priority of partially closed generics</summary>
  public class PartiallyClosedGenericPriorityAugmenter : IActivationStrategyInspector
  {
    /// <summary>Inspect the activation strategy</summary>
    ...
    public void Inspect<T>(T strategy) where T : class, IActivationStrategy
    {
          ...
          configurableStrategy.ActivationType.IsGenericTypeDefinition)
      {
        ...
        foreach (var exportType in configurableStrategy.ExportAs)
        {
          ...
          if (numClosed > maxClosed) { maxClosed = numClosed; }
        }
