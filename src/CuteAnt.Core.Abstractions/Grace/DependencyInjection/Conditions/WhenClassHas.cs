...
{
  /// <summary>condition class for testing if target has a particular attribute</summary>
  public class WhenClassHas : ICompiledCondition
  {
    ...
    /// <summary>Test if strategy meets condition</summary>
  ...
  public class WhenClassHas<TAttribute> : ICompiledCondition where TAttribute : Attribute
  {
    ...
    /// <summary>Should the condition be run at expression creation time or every time a request is made for the type</summary>
    public bool IsRequestTimeCondition { get; } = false;

    /// <summary>If it is a request time condition does it need an injection context</summary>
    ...
    public bool MeetsCondition(IActivationStrategy strategy, StaticInjectionContext staticInjectionContext)
    {
      ...
      if (targetInfo != null)
      {
        foreach (var attribute in targetInfo.InjectionTypeAttributes)
        {
          if (attribute is TAttribute attrT && (_filter == null || _filter(attrT))) { return true; }
        }
      }
