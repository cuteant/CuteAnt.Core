...
namespace CuteAnt.IO
{
    ...
    public static class FileSource
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(filenName)) { return; }
            if (asm == null)
            {
        ...
        public static String FileExists(this Assembly asm, String filenName)
        {
            ...
            if (string.IsNullOrEmpty(filenName)) { ThrowHelper.ThrowArgumentNullException(ExceptionArgument.filenName); }
            ...
            if (ss != null && ss.Length > 0)
            {
                ...
                {
                    name = ss.FirstOrDefault(e => string.Equals(e, filenName, StringComparison.OrdinalIgnoreCase));
                }
                if (string.IsNullOrWhiteSpace(name))
                {
        ...
        {
            if (string.IsNullOrWhiteSpace(filenName)) { return null; }
            ...
            if (checkFileExist)
            {
                ...
                return (!string.IsNullOrWhiteSpace(name)) ? asm.GetManifestResourceStream(name) : null;
            }
            else
            {
