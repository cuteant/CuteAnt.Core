...
{
  /// <summary>A field containing a bound action.</summary>
  ...
  public sealed class BoundActionField<T>
  {
    ...
    /// <summary>Initializes the field with the specified action and context.</summary>
    ...
    /// <summary>Atomically retrieves the bound action from the field and sets the field to <c>null</c>. May return <c>null</c>.</summary>
    public IBoundAction TryGetAndUnset()
    {
      return Interlocked.Exchange(ref _field, null);
    }

    /// <summary>Attempts to update the context of the bound action stored in the field. Returns <c>false</c> if the field is <c>null</c>.</summary>
    ...
    /// <summary>An action delegate bound with its context.</summary>
    public interface IBoundAction
    {
      /// <summary>Executes the action. This should only be done after the bound action is retrieved from a field by <see cref="TryGetAndUnset"/>.</summary>
      void Invoke();
    }
