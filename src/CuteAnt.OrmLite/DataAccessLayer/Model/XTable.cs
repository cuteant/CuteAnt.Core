...
namespace CuteAnt.OrmLite.DataAccessLayer
{
	...
	[Description("表模型")]
	[XmlRoot("Table")]
	internal class OrmLiteTable : IDataTable, ICloneable, IXmlSerializable
	{
		#region 基本属性
		...
		[DisplayName("名称")]
		[Description("名称")]
		public String Name { get { return !_Name.IsNullOrWhiteSpace() ? _Name : (_Name = ModelResolver.Current.GetName(TableName)); } set { _Name = value; } }
		...
		{
			//get { return Columns.FindAll(item => item.PrimaryKey).ToArray(); }
			get { return Columns.Where(item => item.PrimaryKey).ToArray(); }
		}
		...
		public OrmLiteTable(String name)
		{
			TableName = name;
		}
		...
		/// <returns></returns>
		public virtual List<IDataColumn> GetAllColumns(IEnumerable<IDataTable> tables, Boolean baseFirst = true)
		{
		...
		/// <returns></returns>
		public static OrmLiteTable Import(String xml)
		{
			if (xml.IsNullOrWhiteSpace()) { return null; }

			return xml.ToXmlEntity<OrmLiteTable>();
		}
		...
		/// <summary>克隆</summary>
		/// <returns></returns>
		public OrmLiteTable Clone()
		{
			var table = base.MemberwiseClone() as OrmLiteTable;
