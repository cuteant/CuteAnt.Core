	...
	public class ModelResolver : IModelResolver
	{
		...
		public virtual String GetName(IDataColumn dc)
		{
			...
			if (dt != null && AutoCutTableName)
			{
				...
				if (!dt.TableName.IsNullOrWhiteSpace())
				{
					pfs.Add(dt.TableName);
				}
				...
				if (dt.TableName.Contains("_"))
				{
					...
					{
						if (item != null && item.Length >= 2 && !pfs.Contains(item))
						{
							pfs.Add(item);
						}
					}
		...
		private static Boolean IsKeyWord(String name)
		{
			...
			// 特殊处理item，还有CuteAnt和System这两个最重要的命名空间
			if (name.EqualIgnoreCase("item", "System", "CuteAnt", "Eme", "CuteSprite")) { return true; }
		...
		{
			if (description.IsNullOrWhiteSpace()) { return name; }
		...
		protected virtual void FixIndex(IDataTable table)
		{
			...
			foreach (var di in table.Indexes)
			{
				...
				if (!di.Unique)
				{
					di.Unique = dcs.All(dc => dc.Identity);
				}
				if (!di.PrimaryKey)
				{
					di.PrimaryKey = dcs.All(dc => dc.PrimaryKey);
				}
			}
		}
		...
		public static IModelResolver Current
		{
			get { return OrmLiteService.Container.ResolveInstance<IModelResolver>(); }
		}
		...
		{
			get { return _UseID != null ? _UseID.Value : (_UseID = OrmLiteConfig.Current.IsModelUseID).Value; }
			set { _UseID = value; }
		}
		...
		{
			get { return _AutoCutPrefix != null ? _AutoCutPrefix.Value : (_AutoCutPrefix = OrmLiteConfig.Current.IsModelAutoCutPrefix).Value; }
		...
		/// <summary>是否自动去除字段前面的表名。默认启用</summary>
		public Boolean AutoCutTableName
		{
			get { return _AutoCutTableName != null ? _AutoCutTableName.Value : (_AutoCutTableName = OrmLiteConfig.Current.IsModelAutoCutTableName).Value; }
			set { _AutoCutTableName = value; }
		}
		...
		{
			get { return _AutoFixWord != null ? _AutoFixWord.Value : (_AutoFixWord = OrmLiteConfig.Current.IsModelAutoFixWord).Value; }
		...
		/// <summary>要过滤的前缀</summary>
		public String[] FilterPrefixs
		{
			get { return _FilterPrefixs ?? (_FilterPrefixs = OrmLiteConfig.Current.ModelFilterPrefixs.SplitDefaultSeparator<String>(new String[] { "tbl", "table" })); }
			set { _FilterPrefixs = value; }
		}
