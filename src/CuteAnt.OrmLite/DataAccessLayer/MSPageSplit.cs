...
namespace CuteAnt.OrmLite.DataAccessLayer
{
	...
	internal static class MSPageSplit
	{
		...
		/// <returns></returns>
		internal static SelectBuilder PageSplit(SelectBuilder builder, Int64 startRowIndex, Int64 maximumRows, Boolean isSql2005, Func<SelectBuilder, Int64> queryCountCallback = null)
		{
			// 从第一行开始，不需要分页
			...
			{
				throw new OrmLiteException("分页算法要求指定排序列！" + builder.ToString());
			}
			...
			if (isSql2005)
			{
				return RowNumber(builder, startRowIndex, maximumRows);
			}

			// 必须有排序，且排序字段必须就是数字主键
			if (builder.IsInt && builder.KeyIsOrderBy)
			{
			...
			if (maximumRows > 0)
			{
				return DoubleTop(builder, startRowIndex, maximumRows, queryCountCallback);
			}
		...
		private static SelectBuilder TopNotIn(SelectBuilder builder, Int64 startRowIndex, Int64 maximumRows)
		{
			if (builder.Keys == null || builder.Keys.Length != 1)
			{
			...
			if (maximumRows < 1)
			{
				builder2 = builder.CloneWithGroupBy("OrmLite_T0");
			}
			else
			{
			...
			if (!builder2.Column.IsNullOrWhiteSpace() && builder2.Column.Contains("."))
			{
				builder2.Column = "*";
			}
		...
		/// <param name="queryCountCallback">查询总记录数的委托，近供DoubleTop使用</param>
		/// <returns></returns>
		private static SelectBuilder DoubleTop(SelectBuilder builder, Int64 startRowIndex, Int64 maximumRows, Func<SelectBuilder, Int64> queryCountCallback)
		{
			...
			if (builder1.OrderBy.IsNullOrWhiteSpace())
			{
				builder1.OrderBy = builder1.KeyOrder;
			}

			var builder2 = builder1.AsChild("OrmLite_T0").Top(maximumRows);

			// 要反向排序
			builder2.OrderBy = reversekeyorder;

			var builder3 = builder2.AsChild("OrmLite_T1");
			// 结果列处理
			builder3.Column = builder.Column;
			// 如果结果列包含有“.”，即有形如tab1.id、tab2.name之类的列时设为获取子查询的全部列
			if (!builder3.Column.IsNullOrWhiteSpace() && builder3.Column.Contains("."))
			{
				builder3.Column = "*";
			}
			// 让结果正向排序
		...
		/// <param name="maximumRows">最大返回行数，0表示所有行</param>
		/// <returns>分页SQL</returns>
		private static SelectBuilder MaxMin(SelectBuilder builder, Int64 startRowIndex, Int64 maximumRows)
		{
			if (builder.Keys == null || builder.Keys.Length != 1)
			{
				throw new ArgumentNullException("Key", "TopNotIn分页算法要求指定单一主键列！" + builder.ToString());
		...
		/// <param name="maximumRows">最大返回行数，0表示所有行</param>
		/// <returns></returns>
		private static SelectBuilder RowNumber(SelectBuilder builder, Int64 startRowIndex, Int64 maximumRows)
		{
			//if (maximumRows < 1)
			//    sql = String.Format("Select * From (Select *, row_number() over({2}) as rowNumber From {1}) OrmLite_Temp_b Where rowNumber>={0}", startRowIndex + 1, sql, orderBy);
			//else
			//    sql = String.Format("Select * From (Select *, row_number() over({3}) as rowNumber From {1}) OrmLite_Temp_b Where rowNumber Between {0} And {2}", startRowIndex + 1, sql, startRowIndex + maximumRows, orderBy);

			// 如果包含分组，则必须作为子查询
			var builder1 = builder.CloneWithGroupBy("OrmLite_T0");
			...
			var builder2 = builder1.AsChild("OrmLite_T1");
		...
		private static SelectBuilder Top(this SelectBuilder builder, Int64 top, String keyColumn = null)
		{
			if (!keyColumn.IsNullOrWhiteSpace()) { builder.Column = keyColumn; }
			if (builder.Column.IsNullOrWhiteSpace()) { builder.Column = "*"; }
