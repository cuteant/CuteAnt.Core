...
namespace CuteAnt.OrmLite.DataAccessLayer
{
	...
	/// </remarks>
	internal class Distributed : DbBase
	{
		#region - 属性 -
		...
		private const String ExceptionMessage1 = "缺少写入服务器配置WriteServer！";
		private const String ExceptionMessage2 = "缺少读取服务器配置ReadServer！";

		private void LoadConfig(String connStr)
		{
			if (connStr.IsNullOrWhiteSpace()) { return; }
			...
			var ws = "";
			var rs = "";
			if (!builder.TryGetValue("WriteServer", out ws) || ws.IsNullOrWhiteSpace()) { throw new OrmLiteDbException(this, ExceptionMessage1); }
			if (!builder.TryGetValue("ReadServer", out rs) || rs.IsNullOrWhiteSpace()) { throw new OrmLiteDbException(this, ExceptionMessage2); }

			#region 加载写入服务器

			String[] ss = ws.Split(new Char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
			if (ss == null || ss.Length < 1) throw new OrmLiteDbException(this, ExceptionMessage1);
			...
			{
				throw new OrmLiteDbException(this, ExceptionMessage2);
			}
			list.Clear();
		...
		public IDatabase GetReadDb()
		{
			...
			throw new OrmLiteException("设计错误，不应该到达这里！");
		}
