...
namespace CuteAnt.OrmLite.DataAccessLayer
{
  partial class DAL
  {
    ...
    /// <summary>日志</summary>
    public static readonly ILogger Logger = CuteAnt.Log.HmTrace.LoggerFactory.CreateLogger("OrmLite");
    ...
    {
      InitLog();
      Logger.LogError(string.Format(format, args), ex);
    }
    ...
    private static void InitLog()
    {
      ...
      Logger.LogInformation("{0} v{1} Build {2:yyyy-MM-dd HH:mm:ss}", asm.Name, asm.FileVersion, asm.Compile);
      if (DAL.Debug && DAL.NegativeEnable)
      {
        if (DAL.NegativeCheckOnly)
        {
          Logger.LogInformation("OrmLite.Negative.CheckOnly设置为True，只是检查不对数据库进行操作");
        }
        if (DAL.NegativeNoDelete)
        {
          Logger.LogInformation("OrmLite.Negative.NoDelete设置为True，不会删除数据表多余字段");
        }
      }
    }
