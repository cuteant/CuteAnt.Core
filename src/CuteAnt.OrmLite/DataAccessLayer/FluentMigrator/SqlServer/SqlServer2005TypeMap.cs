...
namespace CuteAnt.OrmLite.DataAccessLayer
{
	internal class SqlServer2005TypeMap : SqlServer2000TypeMap
	{
		internal override void SetupTypeMaps()
		{
			...
			SetTypeMap(CommonDbType.AnsiStringFixedLength, "VARCHAR(MAX)", Int32.MaxValue);

			SetTypeMap(CommonDbType.AnsiString, "VARCHAR(MAX)", Int32.MaxValue);
			...
			SetTypeMap(CommonDbType.StringFixedLength, "NVARCHAR(MAX)", Int32.MaxValue);

			// Officially this is 1073741823 but we will allow the int.MaxValue Convention
			SetTypeMap(CommonDbType.String, "NVARCHAR(MAX)", Int32.MaxValue);

			SetTypeMap(CommonDbType.Text, "NVARCHAR(MAX)");
			SetTypeMap(CommonDbType.Json, "NVARCHAR(MAX)");
		}
	}
}