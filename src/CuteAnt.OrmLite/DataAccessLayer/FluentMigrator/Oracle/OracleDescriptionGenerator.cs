...
{
	internal class OracleDescriptionGenerator : GenericDescriptionGenerator<OracleDescriptionGenerator>
	{
		#region Constants
		...
		{
			return schemaName.IsNullOrWhiteSpace() ? tableName : "{0}.{1}".FormatWith(schemaName, tableName);
		}
		...
		internal override String GenerateColumnDescription(String schemaName, String tableName, String columnName, String columnDescription)
		{
			...
			return String.Format(ColumnDescriptionTemplate, GetFullTableName(schemaName, tableName), columnName, columnDescription);
		}
	}
}