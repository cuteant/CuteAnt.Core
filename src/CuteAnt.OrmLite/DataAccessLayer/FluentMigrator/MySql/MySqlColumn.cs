...
{
	internal class MySqlColumn<TTypeMap, TQuoter> : ColumnBase<TTypeMap, TQuoter>
		...
		where TQuoter : MySql55Quoter, new()
	{
		...
		{
			return column.Identity ? "AUTO_INCREMENT" : String.Empty;
		}
		...
		{
			// 统一使用UTF-8字符集
			if (column.DataType == typeof(String)) { return "CHARACTER SET utf8"; }
			return String.Empty;
		}
	}
}