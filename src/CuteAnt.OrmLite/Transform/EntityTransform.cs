...
namespace CuteAnt.OrmLite.Transform
{
  ...
  public class EntityTransform
  {
    ...
    public Int32 TransformTable(IEntityOperate eop, Int64 count = 0, Boolean? isDesc = null, Func<Int32, Int32, IEntityList> getData = null)
    {
      ...
      try
      {
        DAL.Logger.LogInformation("{0} 共 {1}", name, count);
        if (OnlyTransformToEmptyTable && session.Count > 0L)
        {
          DAL.Logger.LogInformation("{0} 非空，跳过", name);
          session.Rollback();
          return 0;
        }
        ...
        while (true)
        {
          ...
          DAL.Logger.LogInformation("{0} 导入 {1}/{2} {3:p}", name, index, count, (Double)index / count);
          total += rs;
        }
      ...
      {
        DAL.Logger.LogError(string.Format("{0} 错误 ", name), ex);
