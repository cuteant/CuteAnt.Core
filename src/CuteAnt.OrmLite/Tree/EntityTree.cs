...
namespace CuteAnt.OrmLite
{
	...
	public abstract partial class EntityTree<TKey, TEntity> : Entity<TEntity>, IEntityTree where TEntity : EntityTree<TKey, TEntity>, new()
	{
		...
		private Int32 Sort
		{
			get { return Setting.Sort.IsNullOrWhiteSpace() ? 0 : (Int32)this[Setting.Sort]; }
			set { if (!Setting.Sort.IsNullOrWhiteSpace() && (Int32)this[Setting.Sort] != value) SetItem(Setting.Sort, value); }
		}
		...
		{
			return FindAllByParent((TKey)this[Setting.Key]);
		}
		...
		public virtual String NodeName
		{
			get
			{
				...
				if (key.IsNullOrWhiteSpace()) return String.Empty;
		...
		public static EntityList<TEntity> FindAllByParent(TKey parentKey)
		{
			...
			if (!Setting.Sort.IsNullOrWhiteSpace())
			{
		...
		public TEntity FindByPath(String path, params String[] keys)
		{
			...
			{
				if (Setting.Name.IsNullOrWhiteSpace()) { return null; }
		...
		/// <summary>已重载，判断两个实体是否相等。有可能是同一条数据的两个实体对象</summary>
		/// <param name="entity"></param>
		/// <returns></returns>
		private Boolean EqualTo(IEntity entity)
		{
			if (entity == null) return false;

			var v1 = this[Setting.Key];
			var v2 = entity[Setting.Key];
			if (typeof(TKey) == TypeX._.String) return "" + v1 == "" + v2;
		...
		{
			return this[Setting.Key].GetHashCode();
		}
		...
		public override void Valid(Boolean isNew)
		{
			...
			if (!pisnull)
			{
				//if (!Meta.Cache.Entities.Exists(KeyName, pkey) && FindCount(KeyName, pkey) <= 0) throw new HmExceptionBase("无效上级[" + pkey + "]！");

				// 先查缓存再查数据库
				var parent = FindByKeyWithCache(pkey);
				if (parent == null) parent = Find(Setting.Key, pkey);
				if (parent == null) throw new EntityException("无效上级[" + pkey + "]！");

				// 检查最大深度
				var maxdeepth = Setting.MaxDeepth;
				if (maxdeepth > 0)
				{
					if (parent.Deepth >= maxdeepth) throw new EntityException("已达到最大深度" + maxdeepth + "层！");
				}
			}
			...
			else
			{
				...
				if (!pisnull && Object.Equals(pkey, key)) throw new EntityException("上级不能是当前节点！");
			}
			...
			if (!isnull && !pisnull)
			{
				...
				{
					throw new EntityException("上级[" + pkey + "]是当前节点的子孙节点！");
				}
			}
		}
		...
		private static Boolean IsNull(TKey value)
		{
			...
			// 字符串的空
			if (typeof(TKey) == typeof(String) && value.ToString().IsNullOrWhiteSpace()) { return true; }
