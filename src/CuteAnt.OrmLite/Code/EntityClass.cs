...
namespace CuteAnt.OrmLite.Code
{
	...
	public sealed class EntityClass
	{
		...
		public String BaseType
		{
			get
			{
				if (_BaseType == null && !Name.IsNullOrWhiteSpace())
				{
					_BaseType = String.Format("Entity<{0}>", Name);
				}
		...
		public void AddProperties()
		{
			...
			foreach (var item in Table.Columns)
			{
				...
				{
					throw new OrmLiteException("[{0}]的[{1}]字段类型DataType不能为空！", Table.DisplayName, item.DisplayName);
				}
				AddField(item);
				AddProperty(item);
			}
		...
		public CodeMemberProperty AddProperty(IDataColumn field)
		{
			...
			p.AddAttribute<BindColumnAttribute>(field.ID, field.ColumnName, field.Description, field.Default == null ? null : field.Default, field.RawType, field.DbType, field.IsUnicode, field.Precision, field.Scale);
		...
		private CodeCommentStatement AddComment(String doc, String comment)
		{
			return new CodeCommentStatement(String.Format("<{0}>{1}</{0}>", doc, comment), true);
		}

		private CodeCommentStatement AddSummary(String comment)
		{
			return AddComment("summary", comment);
		}

		private CodeCommentStatement AddParamComment(String name, String comment)
		{
