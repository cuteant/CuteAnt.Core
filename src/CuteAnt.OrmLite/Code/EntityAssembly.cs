...
namespace CuteAnt.OrmLite.Code
{
	...
	public sealed class EntityAssembly
	{
		...
		{
			get { return !_ConnName.IsNullOrWhiteSpace() ? _ConnName : Name; }
			set { _ConnName = value; }
		}
		...
		public Dictionary<String, String> TypeMaps
		{
		...
		public CodeNamespace NameSpace
		{
			get
			{
				if (_NameSpace == null)
				{
					#region ## 苦竹 修改 2012.12.18 AM 00:48 ##
					//_NameSpace = new CodeNamespace(String.Format("XCode.{0}.Entities", Name));
					_NameSpace = new CodeNamespace(String.Format("CuteAnt.OrmLite.{0}.Entities", Name));
					#endregion
					...
					_NameSpace.Imports.Add(new CodeNamespaceImport("CuteAnt.OrmLite"));
					#endregion
			...
			{
				_NameSpace = value;
				if (_NameSpace != null && !Unit.Namespaces.Contains(_NameSpace))
				{
					Unit.Namespaces.Add(_NameSpace);
				}
			}
		...
		public static EntityAssembly CreateWithCache(String name, List<IDataTable> tables)
		{
			...
			var key = sb.ToString().SHA1().ToHex();
			#endregion
		...
		{
			if (table.Name.IsNullOrWhiteSpace())
			{
		...
		private EntityClass Create2(IDataTable table)
		{
		...
		public void CreateAll()
		{
			...
			#region ## 苦竹 修改 2012.12.18 AM 00:56 ##
			//Unit.AddAttribute<AssemblyTitleAttribute>("XCode动态程序集" + Name);
			//Unit.AddAttribute<AssemblyDescriptionAttribute>("XCode动态生成的实体类程序集");
			Unit.AddAttribute<AssemblyTitleAttribute>("OrmLite动态程序集" + Name);
			Unit.AddAttribute<AssemblyDescriptionAttribute>("OrmLite动态生成的实体类程序集");
			#endregion
		...
		public CompilerResults Compile(CompilerParameters options)
		{
			...
			if (options == null)
			{
				...
				if (Debug)
				{
					...
					var tempPath = HmTrace.TempPath;

					//if (!tempPath.IsNullOrWhiteSpace()) tempPath = Path.Combine(tempPath, Name);
					if (!tempPath.IsNullOrWhiteSpace() && !Directory.Exists(tempPath))
					{
						Directory.CreateDirectory(tempPath);
					}
					#region ## 苦竹 修改 2012.12.18 AM 00:48 ##
					//options.OutputAssembly = Path.Combine(tempPath, String.Format("XCode.{0}.dll", Name));
					options.OutputAssembly = Path.Combine(tempPath, String.Format("CuteAnt.OrmLite.{0}.dll", Name));
					#endregion
			...
			var refs = new String[] { "System.dll", "CuteAnt.OrmLite{0}.dll".FormatWith(AssemblyInfo.VSuffix), "CuteAnt{0}.dll".FormatWith(AssemblyInfo.VSuffix) };
			#endregion
			foreach (var item in AppDomain.CurrentDomain.GetAssemblies())
			{
				...
				if (!options.ReferencedAssemblies.Contains(name))
				{
			...
			{
				// 先生成代码文件，方便调试
				var fileName = Path.Combine(HmTrace.TempPath, Name + ".cs");
				...
				var dllfile = Path.Combine(HmTrace.TempPath, options.OutputAssembly);
		...
		public Assembly Compile()
		{
			...
			{
				return _Assembly = rs.CompiledAssembly;
			}

			//throw new HmCodeException(rs.Errors[0].ErrorText);
		...
		public static Boolean Debug
		{
			get
			{
				...
				_Debug = OrmLiteConfig.Current.IsCodeDebug;
