...
{
  /// <summary>Wrap NLog's Logger in a Microsoft.Extensions.Logging's interface <see cref="Microsoft.Extensions.Logging.ILogger"/>.</summary>
  internal class NLogLogger : Microsoft.Extensions.Logging.ILogger
  {
    ...
    private LogEventInfo CreateLogEventInfo(LogLevel nLogLogLevel, string formattedMessage,
#if NET40
      IList
#else
      IReadOnlyList
#endif
      <KeyValuePair<string, object>> messageProperties, NLogMessageParameterList messageParameters)
    {
    ...
    /// <summary>Checks if the already parsed input message-parameters must be sent through
    /// the NLog MessageTemplate Parser for proper handling of message-template syntax.</summary>
    /// <remarks>Using the NLog MesageTemplate Parser will hurt performance: 1 x Microsoft Parser - 2 x NLog Parser - 1 x NLog Formatter.</remarks>
    private LogEventInfo TryParseMessageTemplate(LogLevel nLogLogLevel,
#if NET40
      IList
#else
      IReadOnlyList
#endif
      <KeyValuePair<string, object>> messageProperties, out NLogMessageParameterList messageParameters)
    {
    ...
    /// <returns></returns>
    private NLogMessageParameterList TryParseMessageParameterList(
#if NET40
      IList
#else
      IReadOnlyList
#endif
      <KeyValuePair<string, object>> messageProperties)
    {
    ...
    private LogEventInfo TryCaptureMessageTemplate(LogLevel nLogLogLevel, string message,
#if NET40
      IList
#else
      IReadOnlyList
    ...
    private LogEventInfo CreateSimpleLogEventInfo(LogLevel nLogLogLevel, string message,
#if NET40
      IList
#else
      IReadOnlyList
    ...
    private void CaptureEventId(LogEventInfo eventInfo, EventId eventId)
    {
      if (!_options.IgnoreEmptyEventId || eventId.Id != 0 || !String.IsNullOrEmpty(eventId.Name))
      {
        ...
        // <see cref="NLogProviderOptions.EventIdSeparator"/>
    ...
    private void CaptureMessagePropertiesList(LogEventInfo eventInfo,
#if NET40
      IList
#else
      IReadOnlyList
    ...
    /// <summary>Is logging enabled for this logger at this <paramref name="logLevel"/>?</summary>
    ...
    /// <summary>Convert loglevel to NLog variant.</summary>
    ...
    /// <summary>Begin a scope. Use in config with ${ndlc}.</summary>
