...
{
	/// <summary>Provides extension methods for <see cref="CancellationToken"/>.</summary>
	internal static class CancellationTokenExtensions
	{
		/// <summary>Returns a <see cref="Task"/> that is canceled when this <see cref="CancellationToken"/> is canceled. 
		/// This method will leak resources if the cancellation token is Int64-lived; 
		/// use <see cref="ToCancellationTokenTaskSource"/> for a similar approach with proper resource management.</summary>
		...
		{
			if (!cancellationToken.CanBeCanceled) { return TaskConstants.Never; }
		...
		/// <summary>Returns a <see cref="CancellationTokenTaskSource"/> which holds the task for this cancellation token.</summary>
		/// <param name="cancellationToken">The cancellation token to observe.</param>
		internal static CancellationTokenTaskSource ToCancellationTokenTaskSource(this CancellationToken cancellationToken)
		{
			return new CancellationTokenTaskSource(cancellationToken);
		}

		/// <summary>Holds the task for a cancellation token, as well as the token registration. 
		/// The registration is disposed when this instance is disposed.</summary>
