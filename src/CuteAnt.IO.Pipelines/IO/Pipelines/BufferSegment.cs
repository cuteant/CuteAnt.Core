...
namespace CuteAnt.IO.Pipelines
{
  internal sealed class BufferSegment : IBufferList<byte>
  {
    ...
    /// Buffer.Length, and must be equal to or less than End.</summary>
    public int Start { get; private set; }

    /// <summary>The End represents the offset into Array where the range of "active" bytes ends. At the point when the block is leased
    ...
    /// <summary>Reference to the next block of data when the overall "active" bytes spans multiple blocks. At the point when the block is
    /// leased Next is guaranteed to be null. Start, End, and Next are used together in order to create a linked-list of discontiguous
    /// working memory. The "active" memory is grown when bytes are copied in, End is increased, and Next is assigned. The "active"
    /// memory is shrunk when bytes are consumed, Start is increased, and blocks are returned to the pool.</summary>
    public BufferSegment NextSegment;

    /// <summary>Combined length of all segments before this.</summary>
