...
{
  /// <summary>Defines a class that provides a pipeline from which data can be read.</summary>
  public interface IPipeReader
  {
    /// <summary>Attempt to synchronously read data the <see cref="IPipeReader"/>.</summary>
    ...
    /// <summary>Asynchronously reads a sequence of bytes from the current <see cref="IPipeReader"/>.</summary>
    ...
    /// <remarks>The memory for the consumed data will be released and no longer available. The examined data
    /// communicates to the pipeline when it should signal more data is available.</remarks>
    ...
    /// <summary>Cancel to currently pending or next call to <see cref="ReadAsync"/> if none is pending,
    ...
    /// <summary>Registers callback that gets executed when writer side of pipe completes.</summary>
