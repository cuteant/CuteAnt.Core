...
namespace CuteAnt.OrmLite.Sprite
{
	...
	public partial class ModelRelation : CommonInt32IdentityPKEntityBase<ModelRelation>, IDataRelation//, IXmlSerializable
	{
		#region 构造

		static ModelRelation()
		{
		}
		...
		protected override bool IsEqualTo(ModelRelation right)
		{
			return ID == right.ID;
		}
		...
		public static ModelRelation FindByID(Int32 id)
		{
			if (id < 1) { return null; }

			if (EntityHelper.IsORMRemoting)
			{
				return SpriteRemotingHeler.SpriteEntities.ModelRelationList.Find(e => id == e.ID);
			}
			else
			{
		...
		/// <returns></returns>
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static EntityList<ModelRelation> FindAllByModelTableID(Int32 modeltableid)
		{
			if (modeltableid < 1) { return null; }

			if (EntityHelper.IsORMRemoting)
			{
				return SpriteRemotingHeler.SpriteEntities.ModelRelationList.FindAll(e => e.ModelTableID == modeltableid);
			}
			else
			{
				var session = Meta.Session;
		...
		/// <summary>根据实体模型、数据列、引用表、引用列查找</summary>
		/// <param name="modeltableid">实体模型</param>
		...
		/// <returns></returns>
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static ModelRelation FindByModelTableIDAndColumnAndRelationTableAndRelationColumn(Int32 modeltableid, String column, String relationtable, String relationcolumn)
		{
			if (EntityHelper.IsORMRemoting)
			{
				return SpriteRemotingHeler.SpriteEntities.ModelRelationList.Find(e => e.ModelTableID == modeltableid && e.Column.EqualIgnoreCase(column) && e.RelationTable.EqualIgnoreCase(relationtable) && e.RelationColumn.EqualIgnoreCase(relationcolumn));
			}
			else
			{
				var session = Meta.Session;
				if (session.EntityCacheDisabled)
				{
					return Find(new String[] { __.ModelTableID, __.Column, __.RelationTable, __.RelationColumn }, new Object[] { modeltableid, column, relationtable, relationcolumn });
				}
				else // 实体缓存
				{
					return session.Cache.Entities.Find(e => e.ModelTableID == modeltableid && e.Column.EqualIgnoreCase(column) && e.RelationTable.EqualIgnoreCase(relationtable) && e.RelationColumn.EqualIgnoreCase(relationcolumn));
				}
			}
		}
		...
		public override void Valid(Boolean isNew)
		{
			...
			if (Column.IsNullOrWhiteSpace()) { throw new ArgumentNullException(__.Column, _.Column.Description + "无效！"); }
			if (RelationTable.IsNullOrWhiteSpace()) { throw new ArgumentNullException(__.RelationTable, _.RelationTable.Description + "无效！"); }
			if (RelationColumn.IsNullOrWhiteSpace()) { throw new ArgumentNullException(__.RelationColumn, _.RelationColumn.Description + "无效！"); }
		...
		//public static EntityList<ModelRelation> Search(Int32 tableid, String key, String orderClause, Int32 startRowIndex, Int32 maximumRows)
		//{
		...
		public static void DeleteCatche(ModelRelation relation)
		{
			if (EntityHelper.IsORMRemoting) { return; }
			if (relation == null) { return; }

			SpriteRemotingHeler.SpriteEntities.IgnoreModelRelationExtendedAttrCache = true;
			SpriteRemotingHeler.SpriteEntities.ModelRelationList.Remove(relation);
			relation = null;
		}
		...
		/// <param name="relation"></param>
		/// <returns></returns>
		internal static ModelRelation Import(Int32 tableid, IDataRelation relation)
		{
			var entity = FindByModelTableIDAndColumnAndRelationTableAndRelationColumn(tableid, relation.Column, relation.RelationTable, relation.RelationColumn);
			if (entity == null)
			{
				entity = new ModelRelation() { ModelTableID = tableid };
			}
			entity.CopyFrom(relation);
			var mr = relation as ModelRelation;
			if (mr != null)
			{
				entity.AllowEdit = mr.AllowEdit;
				entity.AllowDelete = mr.AllowDelete;
			}
		...
		#region IDataRelation 成员

		/// <summary>克隆</summary>
		/// <param name="table"></param>
		/// <returns></returns>
		IDataRelation IDataRelation.Clone(IDataTable table)
		{
			ModelRelation relation = base.MemberwiseClone() as ModelRelation;
			relation.ModelTable = ModelTable;
			return relation;
		}

		/// <summary>数据表</summary>
		[ProtoIgnore, IgnoreDataMember, XmlIgnore]
		IDataTable IDataRelation.Table
		{
