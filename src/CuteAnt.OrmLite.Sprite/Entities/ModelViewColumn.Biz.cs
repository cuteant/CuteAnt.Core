...
{
	/// <summary>视图数据列</summary>
	[ProtoContract(ImplicitFields = ImplicitFields.AllPublic, ImplicitFirstTag = 2)]
	public partial class ModelViewColumn : CommonInt32IdentityPKEntityBase<ModelViewColumn>
	{
		#region 构造

		static ModelViewColumn()
		{
		}
		...
		protected override bool IsEqualTo(ModelViewColumn right)
		{
			return ID == right.ID;
		}
		...
		//    HmTrace.WriteDebug("开始初始化{0}[{1}]数据……", typeof(ModelViewColumn).Name, Meta.Table.DataTable.DisplayName);

		//    var entity = new ModelViewColumn();
		//    entity.ModelViewID = 0;
		//    entity.ColumnName = "abc";
		//    entity.DisplayType = 0;
		//    entity.CustomFormat = "abc";
		//    entity.Sort = 0;
		...
		//    HmTrace.WriteDebug("完成初始化{0}[{1}]数据！", typeof(ModelViewColumn).Name, Meta.Table.DataTable.DisplayName);
		//}
		...
		/// <summary>该视图数据列所对应的模型视图</summary>
		...
		/// <summary>该视图数据列所对应的模型视图视图名称</summary>
		[ProtoIgnore, IgnoreDataMember, XmlIgnore]
		...
		public static EntityList<ModelViewColumn> FindAllByModelViewID(Int32 modelviewid)
		{
			if (modelviewid < 1) { return null; }

			if (EntityHelper.IsORMRemoting)
			{
				var list = SpriteRemotingHeler.SpriteEntities.ModelViewColumnList.FindAll(e => e.ModelViewID == modelviewid);
		...
		/// <summary>根据视图、字段名称查找</summary>
		/// <param name="modelviewid">视图</param>
		/// <param name="columnname">字段名称</param>
		/// <returns></returns>
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static ModelViewColumn FindByModelViewIDAndColumnName(Int32 modelviewid, String columnname)
		{
			if (modelviewid < 1 || columnname.IsNullOrWhiteSpace()) { return null; }

			if (EntityHelper.IsORMRemoting)
			{
				return SpriteRemotingHeler.SpriteEntities.ModelViewColumnList.Find(e => e.ModelViewID == modelviewid && e.ColumnName.EqualIgnoreCase(columnname));
			}
			else
			{
				var session = Meta.Session;
		...
		/// <returns></returns>
		[DataObjectMethod(DataObjectMethodType.Select, false)]
		public static ModelViewColumn FindByID(Int32 id)
		{
			if (id < 1) { return null; }

			if (EntityHelper.IsORMRemoting)
			{
				return SpriteRemotingHeler.SpriteEntities.ModelViewColumnList.Find(e => id == e.ID);
			}
			else
			{
				var session = Meta.Session;
		...
		//public static EntityList<ModelViewColumn> Search(String key, String orderClause, Int32 startRowIndex, Int32 maximumRows)
		//{
		...
		public static void DeleteCatche(ModelViewColumn viewColumn)
		{
			if (EntityHelper.IsORMRemoting) { return; }
			if (viewColumn == null) { return; }

			SpriteRemotingHeler.SpriteEntities.IgnoreModelViewColumnExtendedAttrCache = true;
			SpriteRemotingHeler.SpriteEntities.ModelViewColumnList.Remove(viewColumn);
			viewColumn = null;
		}
