...
{
  /// <summary>Decorator strategy for Func</summary>
  ...
  public class CompiledInitializationDecoratorStrategy<T> : ConfigurableActivationStrategy, ICompiledDecoratorStrategy
  {
    ...
    public CompiledInitializationDecoratorStrategy(Func<T, T> func, IInjectionScope injectionScope)
      : base(typeof(T), injectionScope) => _func = func;
    ...
    public IActivationExpressionResult GetDecoratorActivationExpression(IInjectionScope scope, IActivationExpressionRequest request, ICompiledLifestyle lifestyle)
    {
      ...
      {
        return lifestyle.ProvideLifestyleExpression(scope, request, lifestyleRequest => InternalGetDecoratorActivationExpression(scope, lifestyleRequest));
      }

      return lifestyle.ProvideLifestyleExpression(scope, request, lifestyleRequest => InternalGetDecoratorActivationExpression(scope, request));
    }

    /// <summary>Internal method for creating acivation expression</summary>
    /// <param name="scope"></param>
    /// <param name="request"></param>
    /// <returns></returns>
