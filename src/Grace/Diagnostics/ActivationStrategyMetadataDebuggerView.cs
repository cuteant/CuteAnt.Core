...
{
  /// <summary>Metadata debugger view</summary>
  public class ActivationStrategyMetadataDebuggerView
  {
    ...
    public ActivationStrategyMetadataDebuggerView(IActivationStrategyMetadata metadata) => _metadata = metadata;
    ...
    /// <summary>Export as keyed</summary>
    ...
    /// <summary>Data associated with strategy</summary>
    public IEnumerable<KeyValuePairDebuggerView<object, object>> Data
    {
      get
      {
        var list = _metadata.Select(kvp => new KeyValuePairDebuggerView<object, object>(kvp.Key, kvp.Value)).ToList();
