  ...
  public sealed class SQLiteDataReader : DbDataReader
  {
    ...
    public override Guid GetGuid(int i)
    {
      ...
      if (affinity == TypeAffinity.Blob)
      {
        ...
        return new CombGuid(buffer, CombGuidSequentialSegmentType.Comb).Value;
      }
      else
        return new CombGuid(_activeStatement._sql.GetText(_activeStatement, i), CombGuidSequentialSegmentType.Comb).Value;
    }
    ...
    public CombGuid GetCombGuid(int i)
    {
      ...
      if (affinity == TypeAffinity.Blob)
      {
        byte[] buffer = new byte[16];
        _activeStatement._sql.GetBytes(_activeStatement, i, 0, buffer, 0, 16);
        return new CombGuid(buffer, CombGuidSequentialSegmentType.Comb);
      }
      else
        return new CombGuid(_activeStatement._sql.GetText(_activeStatement, i), CombGuidSequentialSegmentType.Comb);
    }
