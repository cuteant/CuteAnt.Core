namespace Serilog.Exceptions.Destructurers
{
  ...
  public class ReflectionBasedDestructurer : IExceptionDestructurer
  {
    ...
    public void Destructure(Exception exception,
    ...
    private object DestructureValue(object value, int level,
      IDictionary<object, IDictionary<string, object>> destructuredObjects, ref int nextCyclicRefId)
    {
      ...
      if (valueTypeInfo.IsSubclassOf(typeof(MemberInfo))) { return value; }

      if (Type.GetTypeCode(valueType) != TypeCode.Object || valueTypeInfo.IsValueType) { return value; }

      if (level >= MaxRecursiveLevel) { return value; }

      if (typeof(IDictionary).GetTypeInfo().IsAssignableFrom(valueTypeInfo))
      {
        return DestructureValueDictionary(value, level, destructuredObjects, ref nextCyclicRefId);
      }

      if (typeof(IEnumerable).GetTypeInfo().IsAssignableFrom(valueTypeInfo))
      {
        return DestructureValueEnumerable(value, level, destructuredObjects, ref nextCyclicRefId);
      }

      return DestructureObject(value, valueType, level, destructuredObjects, ref nextCyclicRefId);
    }

    private object DestructureValueEnumerable(object value, int level,
      IDictionary<object, IDictionary<string, object>> destructuredObjects, ref int nextCyclicRefId)
    {
      if (destructuredObjects.ContainsKey(value))
      {
        return new Dictionary<string, object>(StringComparer.Ordinal) { { RefLabel, CyclicReferenceMessage } };
      }
      ...
      {
        resultList.Add(DestructureValue(o, level + 1, destructuredObjects, ref nextCyclicRefId));
      }

      return resultList;
    }

    private object DestructureValueDictionary(object value, int level, IDictionary<object,
      IDictionary<string, object>> destructuredObjects, ref int nextCyclicRefId)
    {
      if (destructuredObjects.ContainsKey(value))
      {
        ...
        return new Dictionary<string, object>(StringComparer.Ordinal) { { RefLabel, refId } };
      }
    ...
    private void AppendTypeIfPossible(Dictionary<string, object> values, Type valueType)
    {
      if (values.ContainsKey("Type"))
      {
        ...
        {
          // If both "Type" and "$Type" are present we just give up appending exception type
        }
      }
      else
      {
