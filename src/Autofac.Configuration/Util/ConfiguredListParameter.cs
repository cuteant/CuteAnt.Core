        ...
        private class ListTypeConverter : TypeConverter
        {
            ...
            private static Type GetInstantiableDictionaryType(Type destinationType)
            {
                if (typeof(IDictionary).IsAssignableFrom(destinationType) ||
                    (destinationType.GetTypeInfo().IsConstructedGenericType && typeof(IDictionary<,>).IsAssignableFrom(destinationType.GetGenericTypeDefinition())))
                {
                    var generics = destinationType.GetTypeInfo().IsConstructedGenericType ? destinationType.GetGenericArguments() : new[] { typeof(int), typeof(object) };
            ...
            private static Type GetInstantiableListType(Type destinationType)
            {
                ...
                {
                    var generics = destinationType.GetTypeInfo().IsConstructedGenericType ? destinationType.GetGenericArguments() : new[] { typeof(object) };
