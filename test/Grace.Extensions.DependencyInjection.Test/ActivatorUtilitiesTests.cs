...
{
  public partial class DependencyInjectionSpecificationTests
  {
    ...
    {
      return ActivatorUtils.CreateInstance(provider, type, args);
    }
    ...
    public void TypeActivatorCreateInstanceUsesFirstMathchedConstructor(object value, string ctor)
    {
      ...
      // Act
      var instance = ActivatorUtils.CreateInstance(serviceProvider, type, value);
    ...
    public void GetServiceOrCreateInstanceRegisteredServiceTransient()
    {
      ...
      lock (CreationCountFakeService.InstanceLock)
      {
        ...
        var service = ActivatorUtils.GetServiceOrCreateInstance<CreationCountFakeService>(serviceProvider);
        Assert.NotNull(service);
        Assert.Equal(1, service.InstanceId);
        Assert.Equal(1, CreationCountFakeService.InstanceCount);

        service = ActivatorUtils.GetServiceOrCreateInstance<CreationCountFakeService>(serviceProvider);
        Assert.NotNull(service);
    ...
    public void GetServiceOrCreateInstanceUnregisteredService()
    {
      lock (CreationCountFakeService.InstanceLock)
      {
        ...
        // Act and Assert
        var service = (CreationCountFakeService)ActivatorUtils.GetServiceOrCreateInstance(
            serviceProvider,
