...
namespace Autofac.Test.Core
{
    public class DelegatePropertySelectorTests
    {
        ...
        public void UsesDelegatePassedIn()
        {
            ...
            {
              return Attribute.GetCustomAttributes(propInfo, typeof(InjectPropertyAttribute)).Any();
            });

            foreach (var propInfo in typeof(HasProperties).GetProperties())
            {
        var expected = Attribute.GetCustomAttributes(propInfo, typeof(InjectPropertyAttribute)).Any();
